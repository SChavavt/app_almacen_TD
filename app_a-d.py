...
def mostrar_pedido(df, idx, row, orden, origen_tab, current_main_tab_label, worksheet, headers, s3_client_param,
    ...
    with st.expander(f"{row['Estado']} - {folio} - {row['Cliente']}", expanded=st.session_state["expanded_pedidos"].get(row['ID_Pedido'], False)):  
        ...
        complete_dialog = st.dialog(
            "Confirmar completado",
            key=f"complete_dialog_{row['ID_Pedido']}_{origen_tab}",
        )
        ...
        if complete_dialog.is_open():
            with complete_dialog.container():
                st.warning("¿Confirmar completar el pedido?")
...
def mostrar_pedido_solo_guia(df, idx, row, orden, origen_tab, current_main_tab_label, worksheet, headers, s3_client_param):
    ...
    with st.expander(f"{row['Estado']} - {folio} - {row.get('Cliente','')}", expanded=True):
        ...
        confirm_guia_key = f"confirm_subir_guia_{row['ID_Pedido']}"
        if st.session_state.get(confirm_guia_key):
            st.warning("¿Confirmar subir la guía y completar el pedido?")
            if st.button(
                "✅ Confirmar completado",
                key=f"confirm_subir_guia_only_{row['ID_Pedido']}",
                on_click=preserve_tab_state,
            ):
                st.session_state.pop(confirm_guia_key, None)
            ...
            st.session_state[confirm_guia_key] = True
...
with main_tabs[5]:
    ...
    for _, row in devoluciones_display.iterrows():
        ...
        with st.expander(expander_title, expanded=st.session_state["expanded_devoluciones"].get(row_key, False)):
            ...
            # Botón FINAL: Completar con confirmación
            confirm_devol_key = f"confirm_devol_{folio}_{cliente}"
            if st.session_state.get(confirm_devol_key):
                st.warning("¿Confirmar completar la devolución?")
                if st.button(
                    "✅ Confirmar completado",
                    key=f"confirm_completar_{folio}_{cliente}",
                    on_click=preserve_tab_state,
                ):
                    st.session_state.pop(confirm_devol_key, None)
                    try:
                ...
                st.session_state[confirm_devol_key] = True
...
with main_tabs[6]:  # 🛠 Garantías
    ...
    for _, row in garantias_display.iterrows():
        ...
        with st.expander(expander_title, expanded=st.session_state["expanded_garantias"].get(row_key, False)):
            ...
            confirm_g_key = f"confirm_garantia_{folio}_{cliente}"
            if st.session_state.get(confirm_g_key):
                st.warning("¿Confirmar completar la garantía?")
                if st.button(
                    "✅ Confirmar completado",
                    key=f"confirm_completar_g_{folio}_{cliente}",
                    on_click=preserve_tab_state,
                ):
                    st.session_state.pop(confirm_g_key, None)
                    try:
                ...
                st.session_state[confirm_g_key] = True
