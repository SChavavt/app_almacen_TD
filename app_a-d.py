...
if bulk_mode_value:
    col_bulk_search.text_input(
        ...
        placeholder="Ej. F196697 o ID de pedido",
    )
else:
    ...
    if st.session_state.get("bulk_complete_mode", False):
        ...
        if search_query:
            ...
            if coincidencias.empty:
                st.info(f"No se encontraron pedidos en proceso para '{search_query}'.")
            else:
                st.success(
                    f"Se encontraron {len(coincidencias)} pedido(s) en proceso para '{search_query}'."
                )
                opciones = []
                mapa = {}
