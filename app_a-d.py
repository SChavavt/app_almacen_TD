...
# --- Session State Initialization for Tab Management ---
...
if "tipo_envio_selected_index" not in st.session_state:
    st.session_state.tipo_envio_selected_index = 0
if "estado_pago_selected_index" not in st.session_state:
    st.session_state.estado_pago_selected_index = 0


# --- Tab Definition ---
tab_options = ["🛒 Registrar Nuevo Pedido", "✏️ Modificar Pedido Existente", "🧾 Pedidos Pendientes de Comprobante", "⬇️ Descargar Datos"]
tab1, tab2, tab3, tab4 = st.tabs(tab_options, index=st.session_state.active_main_tab_index)
...
with tab1:
    ...
    # Use session state for the selectbox index
    tipo_envio = st.selectbox(
        "📦 Tipo de Envío",
        ["📍 Pedido Local", "🚚 Pedido Foráneo", "🛠 Garantía", "🔁 Devolución", "📬 Solicitud de guía"],
        index=st.session_state.tipo_envio_selected_index,
        key="tipo_envio_selector_global"
    )
    # Update session state when selection changes
    st.session_state.tipo_envio_selected_index = ["📍 Pedido Local", "🚚 Pedido Foráneo", "🛠 Garantía", "🔁 Devolución", "📬 Solicitud de guía"].index(tipo_envio)
    ...
    if tipo_envio == "📍 Pedido Local":
        ...
        # Use session state for subtipo_local index
        subtipo_local_options = ["☀️ Local Mañana", "🌙 Local Tarde", "🌵 Saltillo", "📦 Pasa a Bodega"]
        subtipo_local = st.selectbox(
            "Turno/Locales",
            subtipo_local_options,
            index=st.session_state.active_subtab_local_index,
            help="Selecciona el turno o tipo de entrega para pedidos locales."
        )
        st.session_state.active_subtab_local_index = subtipo_local_options.index(subtipo_local)
    ...
    with st.form(key="new_pedido_form", clear_on_submit=True):
        ...
        # Use session state for estado_pago index
        estado_pago_options = ["🔴 No Pagado", "✅ Pagado"]
        estado_pago = st.selectbox(
            "💰 Estado de Pago",
            estado_pago_options,
            index=st.session_state.estado_pago_selected_index,
            key="estado_pago_selector_final"
        )
        st.session_state.estado_pago_selected_index = estado_pago_options.index(estado_pago)
        ...
        if submit_button:
            if not vendedor:
                ...
                # Do not rerun
            elif not registro_cliente:
                st.warning("⚠️ Por favor, ingresa el nombre del Cliente.")
                # Do not rerun
            elif estado_pago == "✅ Pagado" and comprobante_pago_file is None:
                ...
                try:
                    ...
                    if not headers:
                        ...
                        if upload_failed:
                            # Do not rerun
                            pass # Already displayed error, do not proceed with sheet update
                        else:
                            ...
                            if upload_failed:
                                ...
                                try:
                                    ...
                                    st.balloons()
                                    st.session_state.active_main_tab_index = tab_options.index("🛒 Registrar Nuevo Pedido") # Stay on current tab
                                    st.rerun() # Rerun only on successful submission
...
with tab2:
    ...
    st.session_state.active_main_tab_index = tab_options.index("✏️ Modificar Pedido Existente") # Update session state
    ...
    if df_pedidos.empty:
        ...
        if filtered_orders.empty:
            ...
            # Store selected order ID in session state to persist selection across reruns
            if 'selected_order_id_mod' not in st.session_state:
                st.session_state.selected_order_id_mod = None

            # Get the index of the previously selected order for the selectbox
            initial_select_index = 0
            if st.session_state.selected_order_id_mod and st.session_state.selected_order_id_mod in filtered_orders['ID_Pedido'].tolist():
                initial_select_index = filtered_orders['ID_Pedido'].tolist().index(st.session_state.selected_order_id_mod)
            elif not filtered_orders.empty:
                st.session_state.selected_order_id_mod = filtered_orders['ID_Pedido'].iloc[0]
            ...
            selected_order_display = st.selectbox(
                ...
                on_change=lambda: setattr(st.session_state, 'selected_order_id_mod', filtered_orders[filtered_orders['display_label'] == st.session_state.select_order_to_modify]['ID_Pedido'].iloc[0]) if st.session_state.select_order_to_modify else None
            )
            ...
            if selected_order_display:
                ...
                # Update selected order in session state for persistence
                st.session_state.selected_order_id_mod = selected_order_id
                ...
                with st.form(key="modify_pedido_form_inner"):
                    ...
                    if modify_button:
                        ...
                        try:
                            ...
                            if 'Modificacion_Surtido' not in headers:
                            ...
                            elif 'Estado_Pago' not in headers:
                            ...
                            elif 'Adjuntos_Surtido' not in headers:
                                ...
                                if changes_made:
                                    ...
                                    st.session_state.active_main_tab_index = tab_options.index("✏️ Modificar Pedido Existente")
                                    st.rerun() # Rerun only if changes were made
                                else:
...
with tab3:
    ...
    st.session_state.active_main_tab_index = tab_options.index("🧾 Pedidos Pendientes de Comprobante") # Update session state
    ...
    if df_pedidos_comprobante.empty:
        ...
        with col3_tab3:
            if 'Vendedor_Registro' in filtered_pedidos_comprobante.columns:
                ...
                if 'selected_vendedor_comp' not in st.session_state:
                    st.session_state.selected_vendedor_comp = "Todos"
                selected_vendedor_comp = st.selectbox(
                    "Filtrar por Vendedor:",
                    options=unique_vendedores_comp,
                    key="comprobante_vendedor_filter",
                    index=unique_vendedores_comp.index(st.session_state.selected_vendedor_comp)
                )
                st.session_state.selected_vendedor_comp = selected_vendedor_comp # Update session state
        ...
        with col4_tab3:
            if 'Tipo_Envio' in filtered_pedidos_comprobante.columns:
                ...
                # Persist filter selection
                if 'selected_tipo_envio_comp' not in st.session_state:
                    st.session_state.selected_tipo_envio_comp = "Todos"
                selected_tipo_envio_comp = st.selectbox(
                    "Filtrar por Tipo de Envío:",
                    options=unique_tipos_envio_comp,
                    key="comprobante_tipo_envio_filter",
                    index=unique_tipos_envio_comp.index(st.session_state.selected_tipo_envio_comp)
                )
                st.session_state.selected_tipo_envio_comp = selected_tipo_envio_comp # Update session state
        ...
        if pedidos_sin_comprobante.empty:
            ...
            # Store selected pending order ID in session state to persist selection
            if 'selected_pending_order_id_comprobante' not in st.session_state:
                st.session_state.selected_pending_order_id_comprobante = None

            initial_pending_select_index = 0
            if st.session_state.selected_pending_order_id_comprobante and st.session_state.selected_pending_order_id_comprobante in pedidos_sin_comprobante['ID_Pedido'].tolist():
                initial_pending_select_index = pedidos_sin_comprobante['ID_Pedido'].tolist().index(st.session_state.selected_pending_order_id_comprobante)
            elif not pedidos_sin_comprobante.empty:
                st.session_state.selected_pending_order_id_comprobante = pedidos_sin_comprobante['ID_Pedido'].iloc[0]
            ...
            selected_pending_order_display = st.selectbox(
                ...
                index=initial_pending_select_index,
                key="select_pending_order_comprobante",
                on_change=lambda: setattr(st.session_state, 'selected_pending_order_id_comprobante', pedidos_sin_comprobante[pedidos_sin_comprobante['display_label'] == st.session_state.select_pending_order_comprobante]['ID_Pedido'].iloc[0]) if st.session_state.select_pending_order_comprobante else None
            )
            ...
            if selected_pending_order_display:
                ...
                # Update selected order in session state for persistence
                st.session_state.selected_pending_order_id_comprobante = selected_pending_order_id
                        ...
                        if comprobante_file_for_pending:
                            try:
                                ...
                                if success_cp:
                                    ...
                                    st.balloons()
                                    st.session_state.active_main_tab_index = tab_options.index("🧾 Pedidos Pendientes de Comprobante")
                                    st.rerun() # Rerun only on successful update
                                else:
...
with tab4:
    ...
    st.session_state.active_main_tab_index = tab_options.index("⬇️ Descargar Datos") # Update session state
    ...
    if df_all_pedidos.empty:
        ...
        # Persist time filter selection
        if 'download_time_filter' not in st.session_state:
            st.session_state.download_time_filter = "Todos los datos"
        time_filter = st.radio(
            "Selecciona un rango de tiempo:",
            ("Todos los datos", "Últimas 24 horas", "Últimos 7 días", "Últimos 30 días"),
            key="download_time_filter"
        )
        # No need to update session state here as st.radio updates it automatically on change
        ...
        if 'Vendedor_Registro' in df_all_pedidos.columns:
            ...
            # Persist filter selection
            if 'selected_vendedor_download' not in st.session_state:
                st.session_state.selected_vendedor_download = "Todos"
            selected_vendedor = st.selectbox(
                "Filtrar por Vendedor:",
                options=options_for_selectbox,
                key="download_vendedor_filter_tab4_final",
                index=options_for_selectbox.index(st.session_state.selected_vendedor_download)
            )
            st.session_state.selected_vendedor_download = selected_vendedor # Update session state
        ...
        if 'Tipo_Envio' in filtered_df_download.columns:
            ...
            # Persist filter selection
            if 'selected_tipo_envio_download' not in st.session_state:
                st.session_state.selected_tipo_envio_download = "Todos"
            selected_tipo_envio_download = st.selectbox(
                "Filtrar por Tipo de Envío:",
                options=unique_tipos_envio_download,
                key="download_tipo_envio_filter",
                index=unique_tipos_envio_download.index(st.session_state.selected_tipo_envio_download)
            )
            st.session_state.selected_tipo_envio_download = selected_tipo_envio_download # Update session state
        ...
        if 'Estado' in filtered_df_download.columns:
            ...
            # Persist filter selection
            if 'selected_estado_download' not in st.session_state:
                st.session_state.selected_estado_download = "Todos"
            selected_estado = st.selectbox(
                "Filtrar por Estado:",
                unique_estados,
                key="download_estado_filter_tab4",
                index=unique_estados.index(st.session_state.selected_estado_download)
            )
            st.session_state.selected_estado_download = selected_estado # Update session state
