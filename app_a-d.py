...
def _ruta_opt_pending_address_hint(pedido: Any, cliente_row: Any = None) -> str:
    parts: list[str] = []
    for source in (cliente_row, pedido):
        if source is None:
            continue
        for field in ("Direccion_Final", "Direccion_Envio", "CalleyNumero", "Col", "Municipio", "C_P."):
            try:
                ...
                pending_clients.append(
                    {
                        ...
                        "pedido": pedido,
                        "motivo": "Sin coordenadas en Clientes_Locales",
                        "direccion_sugerida": _ruta_opt_pending_address_hint(pedido),
                    }
                )
            continue
                ...
                pending_clients.append(
                    {
                        ...
                        "direccion_sugerida": _ruta_opt_pending_address_hint(pedido, cli_row),
                    }
                )
                continue
            ...
            pending_clients.append(
                {
                    "cliente": cliente or "(sin nombre)",
                    "pedido": pedido,
                    "motivo": "Coordenada fuera de zona",
                    "direccion_sugerida": _ruta_opt_pending_address_hint(pedido, cli_row),
                }
            )
            continue
                                ...
                                st.session_state[pending_review_key] = [
                                    {
                                        ...
                                        "direccion_sugerida": str(
                                            item.get("direccion_sugerida", "")
                                            or item.get("pedido", {}).get("Direccion_Envio", "")
