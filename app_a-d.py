...
def _sheet_label_date(fecha: datetime.date) -> str:
    ...
    return f"|{weekday} {fecha.day:02d} DE {month}|"
            ...
            for c in range(df.shape[1]):
                ...
                if current == target:
                    ...
                    for cc in range(c + 1, df.shape[1]):
                        ...
                        if val:
                            return val
                    # 2) Buscar hacia abajo en la misma columna.
                    for rr in range(r + 1, min(df.shape[0], r + 5)):
                        val = _normalize_plain_text(df.iat[rr, c])
                        if val and _remove_accents(val).upper().replace(":", "").strip() != target:
                            return val
                    # 3) Buscar en ventana cercana abajo-derecha (formatos combinados).
                    for rr in range(r + 1, min(df.shape[0], r + 5)):
                        for cc in range(c + 1, min(df.shape[1], c + 5)):
                            val = _normalize_plain_text(df.iat[rr, cc])
                            if val:
                                norm = _remove_accents(val).upper().replace(":", "").strip()
                                if norm != target:
                                    return val
        return ""
    ...
    *,
    ...
    if hasattr(ws, "update_cells"):
        if ":" in a1_range:
            ...
            row_start, col_start = gspread.utils.a1_to_rowcol(a1_range)
            expected_rows_from_values = max(len(values), 1)
            expected_cols_from_values = max((len(row) for row in values), default=1)
            row_end = row_start + expected_rows_from_values - 1
            col_end = col_start + expected_cols_from_values - 1
        expected_rows = row_end - row_start + 1
...
def _match_section_title(cell_value: Any, section_title: str) -> bool:
    ...
    if a == b:
        return True
    # Compatibilidad con secciones previas que incluyen "(MAÑANA-TARDE)".
    a_legacy = a.replace("(MANANA-TARDE)", "").strip()
    b_legacy = b.replace("(MANANA-TARDE)", "").strip()
...
def _append_local_dia_entry_to_hoja_ruta(row: Any, s3_client_param: Any, origen_tab: Any = "") -> bool:
    ...
    if title_row is None:
        ...
        block_rows = HOJA_RUTA_SECTION_TOTAL_ROWS + 1
...
def _upsert_pasa_bodega_report_row(row: Any) -> bool:
    ...
    expected_headers = [
        ...
        "VENDEDOR",
        "ESTADO",
        "FECHA QUE PASO A RECOGER",
        "COMENTARIOS",
    ]
    try:
        headers_read = [str(h or "").strip() for h in ws.row_values(1)]
    except Exception as exc:
        return _fail(f"❌ No se pudieron leer encabezados de Pasa_Bodega: {exc}")

    # Pasa_Bodega tiene un layout fijo A:G. Si gspread lee encabezados incompletos
    # o distintos, no debe bloquear la actualización crítica; se usan posiciones fijas.
    headers = headers_read if all(col in headers_read for col in expected_headers) else expected_headers
    ...
    # Si el registro fue escrito con alguna variación anterior del cliente, usa el
    # folio exacto como respaldo para no anexar duplicados y poder corregir ESTADO/FECHA.
    if target_row is None:
        try:
            all_values = ws.get_all_values()
            for row_idx, row_vals in enumerate(all_values[1:], start=2):
                if _sheet_cell_text(_cell(list(row_vals), num_col_idx)) == target_folio:
                    target_row = row_idx
                    existing_row_values = list(row_vals)
                    break
        except Exception:
    ...
    # luego intenta confirmar exacto por cliente; si no, conserva el folio exacto.
    if target_row is None:
        try:
            col_values = ws.col_values(num_col_idx)
            folio_only_match: tuple[int, list[str]] | None = None
            for i, val in enumerate(col_values[1:], start=2):
                ...
                if _is_exact_pasa_bodega_row(list(row_vals)):
                    ...
                    existing_row_values = list(row_vals)
                    break
                if folio_only_match is None:
                    folio_only_match = (i, list(row_vals))
            if target_row is None and folio_only_match is not None:
                target_row, existing_row_values = folio_only_match
        except Exception:
            target_row = None
            existing_row_values = []
...
def update_gsheet_cell(worksheet, headers, row_index, col_name, value):
    ...
    col_index = _find_header_col_in_row1(worksheet, col_name, headers)
...
_RUTA_OPT_ORIGIN = "KM 3.6 Carretera Nacional, Encino, 64987 Monterrey, N.L., Mexico"
...
def _render_ruta_optimizada_ui(
    *,
    ...
    if st.button("📍 Generar Ruta Optimizada", key=run_key):
        ...
        # CLIENTES usando snap real de Directions (end_location de cada tramo)
        for i, leg in enumerate(legs[:-1], start=1):
            ...
            c = ordered[i - 1] if i - 1 < len(ordered) else {}
            cliente = str(c.get("cliente", f"Cliente {i}"))
            direccion = str(c.get("direccion", ""))

            folium.Marker(
                [lat, lng],
                popup=f"{i}. {cliente}<br>{direccion}",
                tooltip=f"{i}. {cliente}",
        ...
        # REGRESO
        ret_num = len(ordered) + 1
        last_leg = legs[-1] if legs else {}
        end_origin = last_leg.get("end_location", {}) if isinstance(last_leg, dict) else {}
        ret_lat = float(end_origin.get("lat", origin_lat))
        ret_lng = float(end_origin.get("lng", origin_lng))
        folium.Marker(
            [ret_lat, ret_lng],
            popup=f"{ret_num}. REGRESO AL ORIGEN",
            tooltip=f"{ret_num}. REGRESO AL ORIGEN",
        ...
        folium.Marker(
            ...
            icon=folium.features.DivIcon(
                ...
                icon_anchor=(15, 15),
                html=(
                    '<div style="font-size:14px;color:white;background-color:blue;border-radius:50%;'
                    f'width:24px;height:24px;text-align:center;line-height:24px;font-weight:bold;'
                    f'border:2px solid white;">{ret_num}</div>'
                ),
            ),
        ).add_to(m)

        # Dibujar tramos por pasos (color por tramo)
        ...
        for i, leg in enumerate(legs):
            ...
            folium.PolyLine(
                ...
                weight=5,
                opacity=0.9,
                tooltip=f"Tramo {i + 1}: {leg.get('distance', {}).get('text', '')} ({dur_txt})",
            ).add_to(m)
...
def confirmar_modificacion_surtido(
    ...
    def _update_cell_by_col(col_index: int, value: Any, col_name: str) -> bool:
        """Actualiza por índice de columna ya validado para evitar desalinear encabezados."""
        try:
            worksheet.update_cell(gsheet_row_index, col_index, value)
            worksheet_name = _get_worksheet_name_safe(worksheet)
            if worksheet_name:
                _record_local_sheet_update(worksheet_name, int(gsheet_row_index), {col_name: value})
            return True
        except Exception as exc:
    ...
    _dbg(
        ...
        f"col_hora={col_hora} col_fecha_mod={col_fecha_mod}"
    )
    ...
    if len(set(columnas_destino.values())) != len(columnas_destino):
        ...
        return False
    if col_fecha_mod in columnas_destino.values():
        st.error("❌ No se confirmó la modificación para proteger 'Fecha_Modificacion'.")
        _dbg(f"Fecha_Modificacion_colisiona_con_destino={col_fecha_mod}")
        return False

    mod_texto_limpio = str(mod_texto or "").strip()
    texto_confirmado = (
        mod_texto_limpio
        if mod_texto_limpio.endswith("[✔CONFIRMADO]")
        else f"{mod_texto_limpio} [✔CONFIRMADO]".strip()
    )
    ...
    try:
        ...
        before_mod = before_row[col_mod - 1] if len(before_row) >= col_mod else ""
        before_estado = before_row[col_estado - 1] if len(before_row) >= col_estado else ""
        before_hora = before_row[col_hora - 1] if len(before_row) >= col_hora else ""
        before_fecha_mod = before_row[col_fecha_mod - 1] if col_fecha_mod and len(before_row) >= col_fecha_mod else ""
        _dbg(
            "antes | "
    ...
    ok = _update_cell_by_col(col_mod, texto_confirmado, "Modificacion_Surtido")
    if ok:
        ok = _update_cell_by_col(col_estado, "🔵 En Proceso", "Estado")
    if ok:
        ok = _update_cell_by_col(col_hora, hora_confirmacion, "Hora_Proceso")
...
def resolve_caso_gsheet_row_index(df_casos, row, id_pedido, folio, cliente, df_index=None):
    ...
    gsheet_row_idx = None
    try:
        if df_index is not None and not pd.isna(df_index):
            gsheet_row_idx = int(df_index) + 2
    except Exception:
        gsheet_row_idx = None
...
def mostrar_pedido(df, idx, row, orden, origen_tab, current_main_tab_label, worksheet, headers, s3_client_param):
    ...
    guia_marker = "📋 " if (
        es_foraneo_contexto
        and pedido_requiere_guia(row)
        and (not pedido_tiene_guia_adjunta(row))
    ) else ""
        ...
        if not es_local_no_entregado:
            ...
            if is_tab_guias:
                if col_complete_btn.button(
                    ...
                    disabled=disabled_complete_btn,
                    on_click=_mark_skip_demorado_check_once,
                ):
...
if df_main is not None:
    ...
    if (mod_surtido_count > 0) and (not is_sinaicel_topbar):
    ...
    estados_visibles = ["🟡 Pendiente", "🔵 En Proceso", "🔎 Auditado", "🔴 Demorado", "🛠 Modificación", "✏️ Modificación"]
    ...
    is_victor_user_for_view = current_user_for_view == "VICTOR"

    # Aislamiento de datasets por usuario para métricas/avisos:
    # - VICTOR: solo CDMX/Aula.
    # - SINAI/SINAICEL/SCHAVA: excluyen CDMX/Aula.
    ...
    if is_victor_user_for_view:
        ...
        counts_main_activos = _count_states(df_victor_activos)
        counts_main_completados = _count_states(df_victor_completados)
        counts_main = {
            k: counts_main_activos.get(k, 0) + counts_main_completados.get(k, 0)
            for k in ['🟡 Pendiente', '🔵 En Proceso', '🔴 Demorado', '🛠 Modificación', '✏️ Modificación', '🟣 Cancelado', '🟢 Completado']
        }
        counts_casos = {k: 0 for k in ['🟡 Pendiente', '🔵 En Proceso', '🔴 Demorado', '🛠 Modificación', '✏️ Modificación', '🟣 Cancelado', '🟢 Completado']}
    else:
    ...
    pedidos_bodega_demorados = pd.DataFrame(columns=df_main_status_scope.columns)
    if {
        "Tipo_Envio",
        "Turno",
        "Estado",
        "Hora_Proceso",
    }.issubset(df_main_status_scope.columns):
        mask_local_bodega_en_proceso = (
            df_main_status_scope["Tipo_Envio"].astype(str).str.strip().eq("📍 Pedido Local")
            & df_main_status_scope["Turno"].astype(str).str.strip().eq("📦 Pasa a Bodega")
            & df_main_status_scope["Estado"].astype(str).str.strip().eq("🔵 En Proceso")
        )

        if mask_local_bodega_en_proceso.any():
            candidatos = df_main_status_scope.loc[mask_local_bodega_en_proceso].copy()
    ...
    with main_tabs[0]: # 📍 Pedidos Locales
        ...
        def _render_local_turno_subtab(
            *,
            turnos_permitidos: list[str],
            origen_tab: str,
            titulo_turno: str,
            query_param_key: str,
            session_idx_key: str,
            session_label_key: str,
            include_legacy_local_dia: bool = False,
        ) -> None:
            turnos = list(turnos_permitidos)
            if include_legacy_local_dia:
                turnos.append("🌤️ Local Día")

            pedidos_turno_display = df_pendientes_proceso_demorado[
                (df_pendientes_proceso_demorado["Tipo_Envio"] == "📍 Pedido Local")
                & (df_pendientes_proceso_demorado["Turno"].isin(turnos))
            ].copy()

            if pedidos_turno_display.empty:
                st.session_state[session_idx_key] = 0
                st.session_state[session_label_key] = ""
                st.query_params[query_param_key] = "0"
                st.info(f"No hay pedidos para {titulo_turno}.")
                return

            pedidos_turno_display["Fecha_Entrega_dt"] = pd.to_datetime(
                pedidos_turno_display["Fecha_Entrega"],
                errors="coerce",
            )
            estado_entrega_turno = (
                pedidos_turno_display.get("Estado_Entrega", pd.Series(dtype=str))
                .astype(str)
                .str.strip()
            )
            mask_no_entregado_turno = (
                (pedidos_turno_display["Estado"] == "🟢 Completado")
                & (estado_entrega_turno == "⏳ No Entregado")
            )
            pedidos_turno_no_entregado = pedidos_turno_display[
                mask_no_entregado_turno
            ].copy()
            pedidos_turno_activos = pedidos_turno_display[
                ~mask_no_entregado_turno
            ].copy()
            fechas_unicas_dt = sorted(
                pedidos_turno_activos["Fecha_Entrega_dt"].dropna().unique()
            )

            if not fechas_unicas_dt and pedidos_turno_no_entregado.empty:
                st.session_state[session_idx_key] = 0
                st.session_state[session_label_key] = ""
                st.query_params[query_param_key] = "0"
                st.info(f"No hay pedidos para {titulo_turno}.")
            ...
            if not pedidos_turno_no_entregado.empty:
                date_tab_labels = [_LOCAL_NO_ENTREGADOS_TAB_LABEL] + date_tab_labels

            saved_label = st.session_state.get(session_label_key, "")
            fallback_index = (
                date_tab_labels.index(saved_label)
                if saved_label in date_tab_labels
                else _clamp_tab_index(st.session_state.get(session_idx_key, 0), date_tab_labels)
            )
            active_date_tab_index = _resolve_tab_index_from_query(
                st.query_params,
                query_param_key,
                date_tab_labels,
                fallback_index,
            )
            st.session_state[session_idx_key] = active_date_tab_index
            st.session_state[session_label_key] = date_tab_labels[active_date_tab_index]
            st.query_params[query_param_key] = str(active_date_tab_index)

            date_tabs_local = st.tabs(date_tab_labels)
            _emit_recent_tab_group_script(active_date_tab_index, query_param_key)

            for i, tab_label in enumerate(date_tab_labels):
                with date_tabs_local[i]:
                    if tab_label == _LOCAL_NO_ENTREGADOS_TAB_LABEL:
                        st.markdown(
                            f"#### 🚫 Pedidos Locales - {titulo_turno} - No entregados"
                        )
                        if pedidos_turno_no_entregado.empty:
                            st.info("No hay pedidos locales no entregados.")
                        else:
                            fechas_ne_dt = sorted(
                                pedidos_turno_no_entregado["Fecha_Entrega_dt"]
                                .dropna()
                                .unique()
                            )
                            for fecha_dt in fechas_ne_dt:
                                ...
                                pedidos_fecha = pedidos_turno_no_entregado[
                                    pedidos_turno_no_entregado["Fecha_Entrega_dt"] == fecha_dt
                                ].copy()
                            ...
                            pedidos_sin_fecha = pedidos_turno_no_entregado[
                                pedidos_turno_no_entregado["Fecha_Entrega_dt"].isna()
                            ].copy()
                            if not pedidos_sin_fecha.empty:
                                st.markdown("##### 📅 Sin fecha de entrega")
                                pedidos_sin_fecha = ordenar_pedidos_custom(pedidos_sin_fecha)
                                for orden, (idx, row) in enumerate(
        ...
        with subtabs_local[0]:  # ☀️ Local Mañana
            _render_local_turno_subtab(
                turnos_permitidos=["☀️ Local Mañana"],
                origen_tab="Mañana",
                titulo_turno="☀️ Pedidos Locales - Local Mañana",
                query_param_key="local_manana_date_tab",
                session_idx_key="active_date_tab_manana_index",
                session_label_key="active_date_tab_manana_label",
                include_legacy_local_dia=True,
            )

        with subtabs_local[1]:  # 🌙 Local Tarde
            _render_local_turno_subtab(
                turnos_permitidos=["🌙 Local Tarde"],
                origen_tab="Tarde",
                titulo_turno="🌙 Pedidos Locales - Local Tarde",
                query_param_key="local_tarde_date_tab",
                session_idx_key="active_date_tab_tarde_index",
                session_label_key="active_date_tab_tarde_label",
            )
                            ...
                            if mod_confirmation_action:
                                try:
                                    gsheet_row_idx = resolve_caso_gsheet_row_index(
                                        df_casos, row, idp, folio, cliente, df_index=idx
                                    )
        ...
        if pedidos_en_proceso.empty:
            ...
            with st.form("form_completar_rapido", clear_on_submit=False):
                ...
                div[data-testid="stForm"] button[kind="secondaryFormSubmit"] {
                    ...
                    box-shadow: 0 8px 22px rgba(47, 164, 85, 0.34) !important;
                    transition: transform 0.14s ease, box-shadow 0.14s ease, filter 0.14s ease, border-color 0.14s ease !important;
                }
                div[data-testid="stForm"] button[kind="secondaryFormSubmit"]:hover {
                    transform: translateY(-1px) scale(1.01) !important;
                    border-color: rgba(163, 244, 187, 0.98) !important;
                    box-shadow: 0 11px 24px rgba(67, 189, 108, 0.4) !important;
                    filter: brightness(1.05) !important;
                }
                div[data-testid="stForm"] button[kind="secondaryFormSubmit"]:active {
                    transform: translateY(0) scale(0.99) !important;
                    box-shadow: 0 4px 12px rgba(32, 136, 68, 0.38) !important;
                }
                div[data-testid="stForm"] button[kind="secondaryFormSubmit"][disabled] {
                ...
                if run_sinai:
                    ...
                    if selected_df.empty:
                        ...
                        if not api_key:
                            ...
                            candidates, _, _, _, pending_clients, _ = _collect_route_candidates(
                                selected_df,
                                zone_key="local",
                                google_api_key=api_key,
                            ...
                            if candidates:
                                ...
                                # Prioridad suave para los no máximos.
                            ...
                            if not final_candidates:
                                ...
                                if route:
                                    ...
                                    def _format_duracion(minutos: float) -> str:
                                        if minutos >= 60:
                                            total_min_int = int(round(minutos))
                                            horas = total_min_int // 60
                                            mins = total_min_int % 60
                                            return f"{horas}:{mins:02d} horas"
                                    ...
                                    # Agregar regreso al origen (sin salida, solo llegada estimada)
                                    if len(legs) >= len(final_candidates) + 1:
                                        last_leg = legs[len(final_candidates)]
                                        dur_regreso = int(
                                            (
                                                (last_leg.get("duration_in_traffic", {}) or last_leg.get("duration", {}))
                                                or {}
                                            ).get("value", 0)
                                            or 0
                                        )
                                        llegada_origen = cursor_dt + timedelta(seconds=dur_regreso)
                                        eta_rows.append(
                                            {
                                                "Orden": "Fin",
                                                "Cliente": "ORIGEN (regreso)",
                                                "Llegada aprox": llegada_origen.strftime("%H:%M"),
                                                "Salida aprox": "",
                                            }
                                        )
