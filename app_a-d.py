    ...
    if sinai_route_tab_enabled:
        with main_tabs[8]:  # 🗺️ Ruteo
            ...
            if pedidos_locales.empty:
                ...
                grupos_turno = [("Mañana", "☀️ Local Mañana"), ("Tarde", "🌙 Local Tarde"), ("Saltillo", "⛰️ Saltillo")]
                run_sinai = False
                with st.form("sinai_route_form", clear_on_submit=False):
                    selected_rows: list[int] = []
                    priority_rows: list[int] = []
                    for turno_key, title in grupos_turno:
                        st.markdown(f"#### {title}")
                        bloque = pedidos_locales[pedidos_locales.get("Turno", pd.Series(dtype=str)).astype(str).str.strip() == turno_key]
                        if bloque.empty:
