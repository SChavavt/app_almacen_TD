                ...
                if run_sinai:
                    ...
                    if selected_df.empty:
                        ...
                        if not api_key:
                            ...
                            salida_dt = datetime.combine(mx_today(), hora_salida_ruta)
                            ahora_dt = mx_now().replace(tzinfo=None)
                            if salida_dt < ahora_dt - timedelta(minutes=5):
                                # Si eligieron una hora ya pasada hoy, usar mañana para proyectar tráfico.
                                salida_dt = salida_dt + timedelta(days=1)
                            departure_time_unix = int(salida_dt.timestamp())
