        ...
        if pedidos_en_proceso.empty:
            ...
            with st.form("form_completar_rapido", clear_on_submit=False):
                ...
                if foraneos_df.empty:
                    ...
                    grupos = list(foraneos_df.groupby("fecha_lbl", dropna=False, sort=False))
                    col_a, col_b = st.columns(2)
                    for idx_grupo, (fecha_lbl, bloque) in enumerate(grupos):
                        target_col = col_a if idx_grupo % 2 == 0 else col_b
                        with target_col:
