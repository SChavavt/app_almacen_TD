...
if not df_main.empty:
    ...
    with main_tabs[2]:  # 🔁 Devoluciones (casos_especiales)
            ...
            if devoluciones_display.empty:
                ...
                # Enumerar las filas para crear índices únicos
                for row_counter, (row_index, row) in enumerate(devoluciones_display.iterrows()):
                    ...
                    # Emoji para estado de confirmación
                    if confirmado_admin:
                        emoji_confirmacion = "🟢"
                        mensaje_confirmacion = "✅ **CONFIRMADO POR ADMINISTRACIÓN** - Devolución puede viajar"
                    else:
                        emoji_confirmacion = "🟡"
                        mensaje_confirmacion = ""

                    # Título del expander (sin ID del pedido, con Estado_Recepcion y emoji)
                    titulo_expander = f"🔁 {folio or 's/folio'} – {cliente} | Estado: {estado} | Recepción: {estado_recepcion} {emoji_confirmacion}"
                    
                    if confirmado_admin:
                        titulo_expander = f"🔁 {folio or 's/folio'} – {cliente} | {mensaje_confirmacion} {emoji_confirmacion}"

                    with st.expander(titulo_expander, expanded=False):
                        
                        # Mostrar mensaje de confirmación si aplica
                        if confirmado_admin:
                            st.success(mensaje_confirmacion)
                            st.markdown("---")
                        ...
                        with col1:
                            st.markdown(f"**🆔 ID Pedido:** {idp or 'N/A'}")
                            ...
                            st.markdown(f"**🏷️ RFC Cliente:** {str(row.get('Numero_Cliente_RFC', 'N/A')).strip()}")
