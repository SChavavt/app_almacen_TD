...
def debug_alejandro_documento() -> dict:
    ...
    try:
        ss = gspread_client.open_by_key(SPREADSHEET_ID_ALEJANDRO)
        out["open_ok"] = True
        out["title"] = ss.title
        out["url"] = f"https://docs.google.com/spreadsheets/d/{SPREADSHEET_ID_ALEJANDRO}"
...
with tabs[3]:
    ...
    with sub[5]:
        ...
        if st.button("Ejecutar diagnóstico", key="diag_alejandro_doc"):
            ...
            if "not supported for this document" in err.lower():
                st.error(
                    "El ID parece apuntar a un archivo no compatible con Google Sheets API (por ejemplo Excel sin convertir). "
                    "Convierte el archivo a Google Sheets nativo o usa un ID de spreadsheet de Google Sheets."
                )
