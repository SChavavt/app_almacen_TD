...
def cargar_alejandro_todo() -> dict:
    """
    Regresa un dict con DataFrames de todas las hojas del organizador.
    Ej: out["CITAS"] => df de citas
    """
    out = {}
    for nombre in ALE_SHEETS:
        try:
            out[nombre] = cargar_alejandro_hoja(nombre)
        except Exception:
            # Para MVP: no truena toda la app si una hoja no existe o no hay permisos
            out[nombre] = pd.DataFrame(columns=ALE_COLUMNAS.get(nombre, []))
    return out
...
with tabs[3]:
    ...
    ale = cargar_alejandro_todo()

    with sub[0]:
        st.subheader("📌 Hoy")
        df_citas = ale["CITAS"].copy()
        df_tareas = ale["TAREAS"].copy()
        df_cot = ale["COTIZACIONES"].copy()
    ...
    with sub[1]:
        st.subheader("📅 Agenda")
        st.dataframe(ale["CITAS"], use_container_width=True)

    with sub[2]:
        st.subheader("✅ Tareas")
        st.dataframe(ale["TAREAS"], use_container_width=True)

    with sub[3]:
        st.subheader("💰 Cotizaciones")
        st.dataframe(ale["COTIZACIONES"], use_container_width=True)

    with sub[4]:
        st.subheader("🧾 Checklist")
        st.dataframe(ale["CHECKLIST_DAILY"], use_container_width=True)

    with sub[5]:
        st.subheader("⚙️ Config")
        st.dataframe(ale["CONFIG"], use_container_width=True)
