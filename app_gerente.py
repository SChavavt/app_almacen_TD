...
def render_reportes_guia_tab():
    ...
    accion = st.radio(
        "Nuevo valor",
        ["ENTREGADO", "HOY AL FINAL DEL DIA", "SELECCIONAR FECHA"],
        horizontal=True,
        key="reportes_guia_accion",
        help="Al seleccionar una fecha se escribirá en el Excel como día de semana y día, por ejemplo MIERCOLES 10.",
    )
    valor_final = accion
    if accion == "SELECCIONAR FECHA":
        fecha_sel = st.date_input(
            "Selecciona la fecha que quieres escribir en RECIBIDO POR",
            value=now_cdmx().date(),
            format="YYYY-MM-DD",
