        ...
        if base_df.empty:
            ...
            if saldos_df.empty:
                ...
                if not venc_df.empty:
                    ...
                    agenda_df["Codigo"] = agenda_df.get("Codigo", "").astype(str)
                    agenda_df["Folio"] = agenda_df.get("Folio", "").astype(str)
                    agenda_df["Razon_Social"] = agenda_df.get("Razon_Social", "").astype(str)
                    ...
                    if not agenda_df.empty:
                        agenda_df["Etiqueta_Cliente"] = agenda_df["Codigo"] + " · " + agenda_df["Razon_Social"].str.slice(0, 40)
                        agenda_df["Texto_Evento"] = (
                            "Folio " + agenda_df["Folio"]
                            + "<br>Cliente: " + agenda_df["Razon_Social"]
                        ...
                        fig_agenda.update_traces(marker=dict(line=dict(width=0.6, color="rgba(70,70,70,0.35)"), sizemin=8, opacity=0.85))
                        ...
                        calendario_resumen = (
                            agenda_df.assign(Fecha=agenda_df["Fecha_Vencimiento_dt"].dt.strftime("%Y-%m-%d"))
                            .groupby("Fecha", as_index=False)
                            .agg(
                                Folios_Pendientes=("Folio", "nunique"),
                                Clientes=("Codigo", "nunique"),
                                Monto_Del_Dia=("Saldo_Vence", "sum"),
                            )
                            .sort_values("Fecha")
                        )
                        calendario_resumen["Monto_Del_Dia"] = calendario_resumen["Monto_Del_Dia"].map(lambda x: f"${x:,.2f}")
                        with st.expander("📆 Resumen diario (tipo calendario)", expanded=False):
                            st.dataframe(calendario_resumen, use_container_width=True, hide_index=True)
