...
def render_cobranza_tab_gerente():
    ...
    if clientes_mes.empty:
        ...
        if lote_activo:
            ...
            total_folios = len(folios_disponibles)
            todos_seleccionados = total_folios > 0 and len(folios_sel_pre) >= total_folios
            editar_folios = bool(st.session_state.get("ger_cob_folios_edit", False))
            aplicar_filtros = False

            with st.expander("Selección de folios para comentario en lote", expanded=True):
                if todos_seleccionados and not editar_folios:
                    st.info("Ya están seleccionados todos los folios disponibles.")
                    st.caption("Si quieres quitar alguno, usa el botón para editar selección.")
                    if st.button("Editar selección", key="ger_cob_btn_editar_folios"):
                        st.session_state["ger_cob_folios_edit"] = True
                        st.rerun()
                else:
            ...
            if aplicar_filtros:
                st.session_state["ger_cob_folios_edit"] = False
