...
if "organizador" in tab_map:
    with tab_map["organizador"]:
        ...
        with sub[5]:
            ...
            st.markdown("---")
            st.markdown("### 🧑‍💼 Incidencias por mes (solo área responsable: Vendedor)")
            st.caption(
                "Este análisis considera exclusivamente casos donde `Area_Responsable` es exactamente `Vendedor`."
            )
            ...
            if df_vendedor_mes.empty:
                st.info("No hay incidencias con `Area_Responsable = Vendedor` para construir el análisis mensual.")
            else:
                df_vendedor_mes["Mes"] = df_vendedor_mes["Hora_Registro_dt"].dt.to_period("M").astype(str)
                df_vendedor_mes["Mes_Etiqueta"] = df_vendedor_mes["Hora_Registro_dt"].dt.strftime("%Y-%m")
                df_vendedor_mes["Resultado_Esperado_txt"] = (
                    df_vendedor_mes.get("Resultado_Esperado", "").astype(str).str.strip().replace("", "Sin resultado")
                )
                df_vendedor_mes["Responsable_Detalle"] = (
                    df_vendedor_mes["Nombre_Responsable_norm"].astype(str).replace("Sin responsable", "Sin responsable")
                )
                df_vendedor_mes["Monto_Devuelto_num"] = pd.to_numeric(
                    df_vendedor_mes.get("Monto_Devuelto_num", 0), errors="coerce"
                ).fillna(0.0)
                ...
                c_kpi_1.metric("Incidencias vendedor (histórico)", int(len(df_vendedor_mes)))
                c_kpi_2.metric("Meses con incidencias", int(incidencias_mensuales.shape[0]))
                c_kpi_3.metric("Monto devuelto total (vendedor)", f"${df_vendedor_mes['Monto_Devuelto_num'].sum():,.2f}")

                ch_vm_1, ch_vm_2 = st.columns(2)
                with ch_vm_1:
                    st.markdown("#### 📈 Cantidad de incidencias por mes")
                    st.line_chart(incidencias_mensuales)
                with ch_vm_2:
                    st.markdown("#### 💸 Monto devuelto por mes")
                    st.bar_chart(monto_mensual)

                st.markdown("#### 🧾 Lista detallada por mes (Vendedor)")
                resumen_mes = (
                    df_vendedor_mes.groupby("Mes")
                    .agg(
                        Incidencias=("Mes", "size"),
                        Monto_Devuelto_Total=("Monto_Devuelto_num", "sum"),
                        Responsables=("Responsable_Detalle", lambda s: ", ".join(sorted({str(x) for x in s if str(x).strip()}))),
                        Resultados_Esperados=("Resultado_Esperado_txt", lambda s: ", ".join(sorted({str(x) for x in s if str(x).strip()}))),
                    )
                    .sort_index(ascending=False)
                    .reset_index()
                )
                resumen_mes["Monto_Devuelto_Total"] = resumen_mes["Monto_Devuelto_Total"].map(lambda x: f"${x:,.2f}")
                st.dataframe(resumen_mes, use_container_width=True)

                st.markdown("#### 🔎 Top responsables (solo incidencias de vendedor)")
                top_resp_vendedor = (
                    df_vendedor_mes.groupby("Responsable_Detalle")
                    .size()
                    .rename("Incidencias")
                    .sort_values(ascending=False)
                    .head(10)
                )
                if top_resp_vendedor.empty:
                    st.info("No hay responsables para mostrar.")
                else:
                    st.bar_chart(top_resp_vendedor)

                st.markdown("#### 📋 Detalle de casos incluidos")
                columnas_detalle = [
                    "ID_Pedido",
                    "Hora_Registro_dt",
                    "Mes_Etiqueta",
                ...
                columnas_detalle = [c for c in columnas_detalle if c in df_vendedor_mes.columns]
                detalle_vendedor = df_vendedor_mes[columnas_detalle].copy()
                if "Hora_Registro_dt" in detalle_vendedor.columns:
                    detalle_vendedor["Hora_Registro_dt"] = detalle_vendedor["Hora_Registro_dt"].dt.strftime("%d/%m/%Y %H:%M")
                if "Monto_Devuelto_num" in detalle_vendedor.columns:
                    detalle_vendedor["Monto_Devuelto_num"] = detalle_vendedor["Monto_Devuelto_num"].map(lambda x: f"${x:,.2f}")

                detalle_vendedor = detalle_vendedor.rename(
                    columns={
                        "ID_Pedido": "Pedido",
                ...
                st.dataframe(detalle_vendedor.sort_values("Fecha registro", ascending=False), use_container_width=True)
