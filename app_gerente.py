...
def _admintotal_valor_texto(valor):
    """Convierte valores simples o diccionarios de AdminTotal a texto legible."""
    if isinstance(valor, dict):
        for clave in ("nombre", "razon_social", "razón_social", "username", "usuario", "name", "full_name", "email", "codigo"):
...
def _admintotal_primer_valor(factura: dict, claves: list[str]) -> str:
    for clave in claves:
        valor = factura.get(clave)
...
def _admintotal_folio_visual(texto: str) -> str:
    """Formatea FolioSerie como AdminTotal: con prefijo F para folios numéricos."""
...
def _admintotal_folio_serie(factura: dict) -> str:
    folio_serie = _admintotal_primer_valor(factura, ["folio_serie", "folioSerie", "FolioSerie"])
    if folio_serie:
        return _admintotal_folio_visual(folio_serie)

    folio = _admintotal_primer_valor(factura, ["folio"])
    serie = _admintotal_primer_valor(factura, ["serie"])
    if serie and folio:
        return _admintotal_folio_visual(f"{serie}{folio}")
    if folio:
        return _admintotal_folio_visual(folio)

    return _admintotal_folio_visual(_admintotal_primer_valor(factura, ["folio_fiscal", "uuid"]))


def _admintotal_fecha_tiene_hora(texto: str) -> bool:
    """Detecta si un valor de fecha incluye hora además del día."""
    valor = str(texto or "").strip()
    return bool(re.search(r"(?:T|\s)\d{1,2}:\d{2}", valor))


def _admintotal_formatear_fecha_visual(valor: str) -> str:
    """Formatea fechas de AdminTotal para que coincidan con el archivo manual del check."""
    texto = str(valor or "").strip()
    if not texto:
        return ""
    fecha = _parse_fecha_factura_check(texto)
    if pd.isna(fecha):
        return texto
...
def _admintotal_fecha_factura(factura: dict) -> str:
    ...
    return _admintotal_formatear_fecha_visual(valores[0]) if valores else ""


ADMINTOTAL_VENDEDORES_ALIAS = {
    "MICHELLEGLORIAGARCIATORRES": "GARCIA TORRES MICHELLE GLORIA",
    "GLORIAMICHELLEGARCIATORRES": "GARCIA TORRES MICHELLE GLORIA",
    "GARCIATORRESMICHELLEGLORIA": "GARCIA TORRES MICHELLE GLORIA",
    "SANCHEZGARCIACAROLINA": "CAROLINA",
    "GRISELDACAROLINASANCHEZGARCIA": "CAROLINA",
    "CAROLINASANCHEZGARCIA": "CAROLINA",
    "JUANPABLOQUINTERO": "JUAN PABLO QUINTERO",
    "QUINTEROJUANPABLO": "JUAN PABLO QUINTERO",
}

ADMINTOTAL_VENDEDORES_GENERICOS = {
    "CDMXPEDIDOS",
    "PEDIDOSCDMX",
    "USUARIOPEDIDOS",
}
...
def _admintotal_aliases_vendedores() -> dict:
    """Combina alias internos con alias opcionales definidos en Streamlit Secrets."""
    aliases = dict(ADMINTOTAL_VENDEDORES_ALIAS)
    aliases_extra = st.secrets.get("ADMINTOTAL_VENDEDORES_ALIAS", {})
    if isinstance(aliases_extra, Mapping):
        for origen, destino in aliases_extra.items():
            origen_key = normalizar_folio(origen)
            destino_txt = str(destino or "").strip()
            if origen_key and destino_txt:
                aliases[origen_key] = destino_txt
    return aliases


def _admintotal_normalizar_vendedor_alias(nombre: str) -> str:
    """Homologa nombres de vendedor de AdminTotal al formato del reporte manual."""
    texto = str(nombre or "").strip()
    if not texto:
        return ""
    key = normalizar_folio(texto)
    return _admintotal_aliases_vendedores().get(key, texto)


def _admintotal_vendedor_es_generico(nombre: str) -> bool:
    key = normalizar_folio(nombre)
    return key in ADMINTOTAL_VENDEDORES_GENERICOS


def _admintotal_vendedor_factura(factura: dict) -> str:
    """Extrae el vendedor real y evita usar usuarios operativos cuando exista otro campo."""
    claves_preferidas = [
        "agente_nombre",
        "agente",
        "asesor_nombre",
    ...
    claves_respaldo = ["creado_por", "usuario", "usuario_nombre", "created_by", "user"]

    candidatos = []
    for clave in claves_preferidas + claves_respaldo:
        texto = _admintotal_valor_texto(factura.get(clave))
        if texto:
            candidatos.append(texto)

    for texto in candidatos:
        if not _admintotal_vendedor_es_generico(texto):
            return _admintotal_normalizar_vendedor_alias(texto)
    return _admintotal_normalizar_vendedor_alias(candidatos[0]) if candidatos else ""


def _admintotal_factura_a_fila(factura: dict) -> dict:
    return {
        "Vendedor": _admintotal_vendedor_factura(factura),
        "FolioSerie": _admintotal_folio_serie(factura),
        "Cliente": _admintotal_primer_valor(factura, ["cliente", "cliente_razon_social", "razon_social", "receptor_nombre"]),
        "Fecha": _admintotal_fecha_factura(factura),
    }


def consultar_facturas_admintotal(params):
    """Consulta facturas de AdminTotal y devuelve un DataFrame limitado a 1000 registros."""
    token_resultado = obtener_token_admintotal()
    if not token_resultado.get("success"):
        return {
            "success": False,
            "status_code": token_resultado.get("status_code"),
            "error": token_resultado.get("error", "No fue posible obtener token de AdminTotal."),
            "df": pd.DataFrame(columns=["Vendedor", "FolioSerie", "Cliente", "Fecha"]),
        }

    base_url = token_resultado.get("base_url", "").rstrip("/")
    url = f"{base_url}/api/v2/movimientos/facturas/"
    headers = {"Authorization": f"Bearer {token_resultado.get('access_token', '')}"}
    params_limpios = {k: v for k, v in dict(params or {}).items() if v not in (None, "")}
    facturas = []
    status_code = None

    try:
        while url and len(facturas) < 1000:
            response = requests.get(url, headers=headers, params=params_limpios, timeout=30)
            status_code = response.status_code
            if not response.ok:
                try:
                    data_error = response.json()
                    detalle_error = data_error.get("detail") or data_error.get("error") or str(data_error)[:300]
                except ValueError:
                    detalle_error = response.text[:300]
                return {
                    "success": False,
                    "status_code": status_code,
                    "error": detalle_error or "Error al consultar facturas en AdminTotal.",
                    "df": pd.DataFrame(columns=["Vendedor", "FolioSerie", "Cliente", "Fecha"]),
                }

            data = response.json()
            if isinstance(data, dict):
                resultados = data.get("results") or data.get("data") or data.get("facturas") or []
            ...
            for factura in resultados:
                if isinstance(factura, dict):
                    facturas.append(_admintotal_factura_a_fila(factura))
                    if len(facturas) >= 1000:
                        break

            if siguiente and len(facturas) < 1000:
                url = urllib.parse.urljoin(base_url, siguiente)
            else:
                url = None
            params_limpios = None

        df = pd.DataFrame(facturas, columns=["Vendedor", "FolioSerie", "Cliente", "Fecha"])
        return {"success": True, "status_code": status_code, "df": df}
    except requests.RequestException as exc:
...
def _normalizar_nombre_columna_admintotal(col_name: str) -> str:
    """Normaliza nombres de columnas/campos para comparar exportaciones y respuestas API."""
    col = str(col_name or "").strip().lower()
    col = unicodedata.normalize("NFKD", col).encode("ascii", "ignore").decode("ascii")
    return re.sub(r"\s+", " ", col)


def _extraer_existencias_desde_excel_admintotal(content: bytes) -> pd.DataFrame:
    """Lee el Excel de Productos por Almacén de AdminTotal y valida que la columna I sea Disponible."""
    xls = pd.ExcelFile(BytesIO(content))
    hoja = xls.sheet_names[0]

    for header_row in range(0, 8):
        df_try = pd.read_excel(BytesIO(content), sheet_name=hoja, header=header_row)
        cols_norm = {_normalizar_nombre_columna_admintotal(c): c for c in df_try.columns}
        if "codigo" not in cols_norm:
            continue

        col_codigo = cols_norm["codigo"]
        if len(df_try.columns) < 9:
            raise ValueError("El Excel de AdminTotal debe incluir la columna I con encabezado Disponible.")

        col_disponible = df_try.columns[8]
        col_disponible_norm = _normalizar_nombre_columna_admintotal(col_disponible)
        if col_disponible_norm != "disponible":
            raise ValueError(
                "La columna I del Excel consultado no se llama Disponible. "
                f"Encabezado detectado en columna I: {col_disponible!r}."
            )

        df = df_try[[col_codigo, col_disponible]].copy()
        df.columns = ["Código", "Disponible"]
        df = df.dropna(subset=["Código"], how="all")
        df["Código"] = df["Código"].astype(str).str.strip()
        df = df[df["Código"] != ""]
        df["Disponible"] = pd.to_numeric(df["Disponible"], errors="coerce").fillna(0)
        df = df[["Código", "Disponible"]]
        df.attrs["admintotal_meta"] = {
            "formato": "XLSX",
            "hoja": hoja,
            "fila_encabezado": header_row + 1,
            "columna_codigo": str(col_codigo),
            "columna_disponible": str(col_disponible),
            "columna_disponible_excel": "I",
            "columnas_detectadas": [str(c) for c in df_try.columns],
        }
        return df

    raise ValueError("El Excel de AdminTotal no contiene encabezados reconocibles con Código y Disponible (columna I).")


def _admintotal_productos_params() -> dict:
    """Parámetros por defecto para replicar la exportación de Productos por Almacén (001 Matriz)."""
    params = {
        "f": st.secrets.get("ADMINTOTAL_PRODUCTOS_F", "1"),
        "proveedor": st.secrets.get("ADMINTOTAL_PRODUCTOS_PROVEEDOR", ""),
        "status_producto": st.secrets.get("ADMINTOTAL_PRODUCTOS_STATUS", "1"),
        "almacen": st.secrets.get("ADMINTOTAL_ALMACEN_ID", "9"),
        "linea": st.secrets.get("ADMINTOTAL_PRODUCTOS_LINEA", ""),
        "sublinea": st.secrets.get("ADMINTOTAL_PRODUCTOS_SUBLINEA", ""),
        "subsublinea": st.secrets.get("ADMINTOTAL_PRODUCTOS_SUBSUBLINEA", ""),
        "choices": st.secrets.get("ADMINTOTAL_PRODUCTOS_CHOICES", ""),
        "page_size": st.secrets.get("ADMINTOTAL_PRODUCTOS_PAGE_SIZE", "500"),
        "buscar": st.secrets.get("ADMINTOTAL_PRODUCTOS_BUSCAR", "Buscar"),
    }
    params_extra_raw = st.secrets.get("ADMINTOTAL_PRODUCTOS_PARAMS", "")
    if params_extra_raw:
        try:
            params.update(json.loads(params_extra_raw) if isinstance(params_extra_raw, str) else dict(params_extra_raw))
        except Exception:
            pass
    return params


def _admintotal_productos_registros(data):
    """Obtiene lista de productos desde respuestas paginadas o listas simples."""
    if isinstance(data, list):
        return data, None
    if not isinstance(data, dict):
        return [], None
    for key in ("results", "data", "productos", "objects", "items"):
        value = data.get(key)
        if isinstance(value, list):
            return value, data.get("next")
    return [], data.get("next")


def _existencias_desde_registros_admintotal(registros: list[dict]) -> pd.DataFrame:
    """Convierte registros JSON de AdminTotal a Código/Disponible, exigiendo campo Disponible."""
    filas = []
    codigo_norm_permitidos = {"codigo", "modelo", "clave", "sku"}
    columnas_json = set()
    ...
    for registro in registros:
        if not isinstance(registro, dict):
            continue
        columnas_json.update(str(k) for k in registro.keys())
        codigo = ""
        disponible = None
        for key, valor in registro.items():
            key_norm = _normalizar_nombre_columna_admintotal(key)
            if key_norm in codigo_norm_permitidos and valor not in (None, "") and not codigo:
                codigo = str(valor).strip()
            if key_norm == "disponible":
                disponible = valor
                tiene_disponible = True
        if not codigo:
            continue
        filas.append({"Código": codigo, "Disponible": disponible})

    if filas and not tiene_disponible:
        raise ValueError("La respuesta JSON de AdminTotal no contiene el campo Disponible.")

    df = pd.DataFrame(filas, columns=["Código", "Disponible"])
    if df.empty:
        return df
    df["Disponible"] = pd.to_numeric(df["Disponible"], errors="coerce").fillna(0)
    df.attrs["admintotal_meta"] = {
        "formato": "JSON",
        "columna_codigo": "Código/modelo/clave/sku",
        "columna_disponible": "Disponible",
        "columnas_detectadas": sorted(columnas_json),
    }
    return df


def consultar_existencias_admintotal_productos(
    request_timeout: int = 20,
    total_timeout: int = 45,
    max_pages: int = 20,
    require_configured_endpoint: bool = False,
) -> dict:
    """
    Consulta AdminTotal para traer existencias del catálogo Productos por Almacén.

    Si el endpoint configurado devuelve XLSX/CSV, se lee como la exportación manual y se usa la columna I
    (Disponible). Si devuelve JSON, se toman los campos equivalentes a código y disponible.
    Los límites de tiempo/páginas evitan que la UI se quede cargando indefinidamente.
    """
    token_resultado = obtener_token_admintotal()
    if not token_resultado.get("success"):
        return {"success": False, "error": token_resultado.get("error", "No fue posible obtener token de AdminTotal."), "df": pd.DataFrame()}

    base_url = token_resultado.get("base_url", "").rstrip("/")
    endpoint_config = str(st.secrets.get("ADMINTOTAL_PRODUCTOS_ALMACEN_ENDPOINT", "")).strip()
    if require_configured_endpoint and not endpoint_config:
        return {
            "success": False,
            "error": (
                "Para comparar rápido sin quedarse cargando, configura primero "
                "ADMINTOTAL_PRODUCTOS_ALMACEN_ENDPOINT con la ruta exacta del export/API de Productos por Almacén."
            ),
            "df": pd.DataFrame(),
        }
    endpoints = [endpoint_config] if endpoint_config else [
        "/api/v2/inventario/catalogos/productos_almacen/",
        "/api/v2/inventario/productos_almacen/",
        "/api/v2/productos_almacen/",
    ]
    headers = {
        "Authorization": f"Bearer {token_resultado.get('access_token', '')}",
        "Accept": "application/json, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, text/csv, */*",
    }
    params_base = _admintotal_productos_params()
    errores = []
    inicio_consulta = time.monotonic()

    def _validar_tiempo_disponible():
        if total_timeout and (time.monotonic() - inicio_consulta) > total_timeout:
            raise TimeoutError(
                f"La consulta de AdminTotal excedió {total_timeout} segundos. "
                "Revisa/configura la ruta exacta del export para evitar búsquedas lentas."
            )

    for endpoint in endpoints:
        if not endpoint:
            continue
        url = endpoint if endpoint.lower().startswith("http") else f"{base_url}/{endpoint.lstrip('/')}"
        params = dict(params_base)
        registros = []
        status_code = None
        try:
            pagina_actual = 0
            while url and len(registros) < 20000:
                _validar_tiempo_disponible()
                if max_pages and pagina_actual >= max_pages:
                    raise TimeoutError(
                        f"AdminTotal devolvió más de {max_pages} páginas. "
                        "Sube el page_size o usa el endpoint exacto de export XLSX para comparar más rápido."
                    )
                pagina_actual += 1
                ...
                if not response.ok:
                    errores.append(f"{endpoint}: HTTP {status_code} {response.text[:180]}")
                    break

                if "spreadsheet" in content_type or response.content[:2] == b"PK":
                    df = _extraer_existencias_desde_excel_admintotal(response.content)
                    metadata = dict(df.attrs.get("admintotal_meta", {}))
                    metadata.update({"url_consultada": response.url, "endpoint_configurado": endpoint})
                    df.attrs["admintotal_meta"] = metadata
                    return {"success": True, "status_code": status_code, "df": df, "source_url": response.url, "metadata": metadata}
                if "csv" in content_type:
                    df_csv = pd.read_csv(BytesIO(response.content))
                    cols_norm = {_normalizar_nombre_columna_admintotal(c): c for c in df_csv.columns}
                    if "codigo" in cols_norm and len(df_csv.columns) >= 9:
                        col_disp = df_csv.columns[8]
                        if _normalizar_nombre_columna_admintotal(col_disp) != "disponible":
                            raise ValueError(
                                "La columna I del CSV consultado no se llama Disponible. "
                                f"Encabezado detectado en columna I: {col_disp!r}."
                            )
                        df = df_csv[[cols_norm["codigo"], col_disp]].copy()
                        df.columns = ["Código", "Disponible"]
                        df["Disponible"] = pd.to_numeric(df["Disponible"], errors="coerce").fillna(0)
                        metadata = {
                            "formato": "CSV",
                            "url_consultada": response.url,
                            "endpoint_configurado": endpoint,
                            "columna_codigo": str(cols_norm["codigo"]),
                            "columna_disponible": str(col_disp),
                            "columna_disponible_excel": "I",
                            "columnas_detectadas": [str(c) for c in df_csv.columns],
                        }
                        df.attrs["admintotal_meta"] = metadata
                        return {"success": True, "status_code": status_code, "df": df, "source_url": response.url, "metadata": metadata}

                data = response.json()
                nuevos, siguiente = _admintotal_productos_registros(data)
                registros.extend(nuevos)
                if siguiente:
                    url = urllib.parse.urljoin(base_url, str(siguiente))
                    params = None
                else:
                    url = None

            df = _existencias_desde_registros_admintotal(registros)
            if not df.empty:
                metadata = dict(df.attrs.get("admintotal_meta", {}))
                metadata.update({"url_consultada": endpoint, "endpoint_configurado": endpoint})
                df.attrs["admintotal_meta"] = metadata
                return {"success": True, "status_code": status_code, "df": df, "source_url": endpoint, "metadata": metadata}
        except Exception as exc:
            errores.append(f"{endpoint}: {exc}")

    return {
        "success": False,
        "error": "No se pudieron consultar existencias de AdminTotal. " + " | ".join(errores[-3:]),
        "df": pd.DataFrame(),
    }
...
def render_prueba_admintotal_tab(button_key: str = "btn_probar_admintotal"):
    ...
    st.info("Esta pestaña temporal permite probar el token JWT y consultar facturas con columnas limitadas.")
    ...
    st.divider()
    st.subheader("🧾 Consultar facturas AdminTotal")
    hoy = now_cdmx().date()
    col_fecha_desde, col_fecha_hasta = st.columns(2)
    with col_fecha_desde:
        fecha_desde = st.date_input("Fecha desde", value=hoy, key=f"{button_key}_facturas_desde")
    with col_fecha_hasta:
        fecha_hasta = st.date_input("Fecha hasta", value=hoy, key=f"{button_key}_facturas_hasta")

    col_folio, col_cliente, col_vendedor = st.columns(3)
    with col_folio:
        folio = st.text_input("Folio opcional", key=f"{button_key}_facturas_folio").strip()
    with col_cliente:
        cliente = st.text_input("Cliente opcional", key=f"{button_key}_facturas_cliente").strip()
    with col_vendedor:
        vendedor = st.text_input("Vendedor opcional", key=f"{button_key}_facturas_vendedor").strip()

    if st.button("🧾 Consultar facturas AdminTotal", key=f"{button_key}_consultar_facturas"):
        ...
        resultado_facturas = consultar_facturas_admintotal(params)
        if not resultado_facturas.get("success"):
            st.error(f"❌ Error al consultar facturas: {resultado_facturas.get('error', 'Error desconocido')}")
            if resultado_facturas.get("status_code") is not None:
                st.write(f"**Status code:** {resultado_facturas.get('status_code')}")
            return

        df_facturas = resultado_facturas.get("df", pd.DataFrame(columns=["Vendedor", "FolioSerie", "Cliente", "Fecha"]))
        if vendedor:
            vendedor_norm = vendedor.lower()
            df_facturas = df_facturas[df_facturas["Vendedor"].astype(str).str.lower().str.contains(vendedor_norm, na=False)].copy()

        st.write(f"**Total de facturas encontradas:** {len(df_facturas)}")
        if df_facturas.empty:
            st.warning("No se encontraron facturas con los filtros seleccionados.")
            return

        df_facturas = df_facturas[["Vendedor", "FolioSerie", "Cliente", "Fecha"]]
...
def _mail_error_hint(exc: Exception) -> str:
    ...
    if isinstance(exc, urllib.error.HTTPError):
        ...
        body_l = body.lower()
        if status == 401:
            return "401 Unauthorized: API key inválida o sin permisos de Mail Send."
        if status == 403:
            if "from" in body_l and ("verified" in body_l or "sender" in body_l):
                return "403 Forbidden: el remitente (from_email) no está verificado en SendGrid (Single Sender o Domain Authentication)."
            return "403 Forbidden: revisa permisos de la API key y verificación del remitente en SendGrid."
        if status == 429:
            return "429 Too Many Requests: límite temporal de SendGrid, intenta de nuevo en unos minutos."
        if status and status >= 500:
            return f"{status} Error de SendGrid: fallo temporal del servicio."
        if body:
            return f"HTTP {status}: {body[:260]}"
        return f"HTTP {status}: no fue posible enviar el correo."
    return str(exc)
...
ADMINTOTAL_CHECK_FACTURAS_DIAS = 4
ADMINTOTAL_CHECK_FACTURAS_VENTANA_ANALISIS_HORAS = 72
...
def _parse_fecha_factura_check(valor):
    """Parsea fechas de facturas en formato AdminTotal ISO o archivo manual DD/MM/YYYY."""
    if valor in (None, ""):
        return pd.NaT
    ...
    if isinstance(valor, pd.Timestamp):
        ...
        if not texto:
            return pd.NaT
        # AdminTotal devuelve fechas como YYYY-MM-DD; con dayfirst=True pandas las interpreta mal
        # (ej. 2026-06-02 puede quedar como 2026-02-06). Para esos casos parseamos ISO primero.
    ...
    if getattr(fecha, "tzinfo", None) is not None:
        try:
            fecha = fecha.tz_convert(MEXICO_CITY_TZ).tz_localize(None)
        except TypeError:
...
def analizar_facturas_check_desde_df(df_facturas_input: pd.DataFrame, *, ventana_horas: int = 96) -> dict:
    ...
    df_facturas = df_facturas_input.copy() if isinstance(df_facturas_input, pd.DataFrame) else pd.DataFrame(columns=columnas)
    for col in columnas:
    ...
    limite_ventana = ahora_naive - timedelta(hours=ventana_horas)

    df_facturas["_fecha_factura_dt"] = df_facturas["Fecha"].apply(_parse_fecha_factura_check)
    df_facturas = df_facturas[df_facturas["_fecha_factura_dt"].notna()].copy()
    df_facturas = df_facturas[
        (df_facturas["_fecha_factura_dt"] >= limite_ventana)
    ...
    df_pedidos_match = pd.concat([df_pedidos_match, df_casos_match], ignore_index=True, sort=False)
    ...
    df_pedidos_match["_folios_factura_set"] = df_pedidos_match["Folio_Factura"].apply(
        lambda v: extraer_folios_posibles(v) or ({normalizar_folio_para_match(v)} if normalizar_folio_para_match(v) else set())
    )
    ...
    if col_hora_registro is None:
        ...
        df_pedidos_match["_hora_registro_dt"] = pd.to_datetime(df_pedidos_match[col_hora_registro], errors="coerce")
    ...
    if col_cliente_sistema is None:
        ...
        df_pedidos_match["_cliente_norm"] = df_pedidos_match[col_cliente_sistema].astype(str).apply(normalizar).str.strip()
    ...
    progreso_match = st.progress(0, text=f"Analizando facturas... 0/{total_a_analizar}")
    ...
    for idx, (_, fila_factura) in enumerate(df_facturas.iterrows(), start=1):
        ...
        candidatos_folio = df_pedidos_match[
            df_pedidos_match["_folios_factura_set"].apply(lambda s: str(folio_factura).strip() in s)
        ].copy()
        ...
        if cliente_factura and not match_folio_con_fecha:
            candidatos_cliente = df_pedidos_match[
                df_pedidos_match["_cliente_norm"].astype(str).apply(lambda c: coincide_nombre_cliente(cliente_factura, c))
            ].copy()
        ...
        progreso_match.progress(porcentaje, text=f"Analizando facturas... {idx}/{total_a_analizar}")
        if idx == total_a_analizar or idx % 25 == 0:
            estado_match.caption(f"Procesadas {idx} de {total_a_analizar} facturas.")
    ...
    df_no_encontradas = (
        df_facturas[
            ~(df_facturas["_match_valido_folio"] | df_facturas["_match_valido_adjuntos"] | df_facturas["_match_valido_cliente_sin_folio"])
        ]
    ...
    return {
        ...
        "df_match_cliente_sin_folio": df_match_cliente_sin_folio,
    }


def mostrar_resultado_check_facturas(resultado_check: dict, *, firma_guardado: str, filtro_key: str, guardado_sig_key: str):
    """Muestra y guarda el resultado del check de facturas."""
    df_no_encontradas = resultado_check.get("df_no_encontradas", pd.DataFrame(columns=FACTURAS_FALTANTES_COLUMNAS)).copy()
    df_match_cliente_sin_folio = resultado_check.get("df_match_cliente_sin_folio", pd.DataFrame(columns=FACTURAS_FALTANTES_COLUMNAS)).copy()
    total_archivo = int(resultado_check.get("total_archivo", 0))
    total_recibidas = int(resultado_check.get("total_recibidas", total_archivo))
    total_no_encontradas = int(resultado_check.get("total_no_encontradas", len(df_no_encontradas)))
    limite_ventana = resultado_check.get("limite_ventana")
    ahora_naive = resultado_check.get("ahora_naive")
    ventana_horas = int(resultado_check.get("ventana_horas", 72))

    if st.session_state.get(guardado_sig_key) != firma_guardado:
        ok_guardado_ff, msg_guardado_ff = guardar_facturas_faltantes_en_sheet(df_no_encontradas)
        if ok_guardado_ff:
            st.session_state[guardado_sig_key] = firma_guardado
            st.success(f"✅ {msg_guardado_ff}")
        else:
            st.error(f"❌ {msg_guardado_ff}")

    vendedores_disponibles_check = sorted(
        {str(v).strip() for v in df_no_encontradas.get("Vendedor", pd.Series(dtype=str)).dropna().tolist() if str(v).strip()}
    )
    ...
    st.info(
        f"Facturas recibidas: {total_recibidas} | "
        f"Facturas analizadas: {total_archivo} | "
        f"No encontradas en sistema: {total_no_encontradas}"
    )
    if limite_ventana is not None and ahora_naive is not None:
        st.caption(
            f"Filtro aplicado: últimas {ventana_horas} horas ({limite_ventana.strftime('%d/%m/%Y %H:%M')} "
    ...
    if total_archivo == 0:
        st.info(f"No hay facturas en las últimas {ventana_horas} horas para validar.")
    elif total_no_encontradas == 0:
        st.success("✅ Todas las facturas cumplen match válido por folio o por cliente dentro de la ventana definida.")
    else:
        st.error(f"❌ {total_no_encontradas} factura(s) no se encontraron en el sistema.")
        st.dataframe(df_no_encontradas_filtrado, use_container_width=True, hide_index=True)
        output_ff = BytesIO()
        with pd.ExcelWriter(output_ff, engine="xlsxwriter") as writer:
            df_no_encontradas[FACTURAS_FALTANTES_COLUMNAS].to_excel(writer, index=False, sheet_name="Facturas_Faltantes")
        st.download_button(
            "⬇️ Descargar facturas no encontradas",
            data=output_ff.getvalue(),
            file_name="facturas_no_encontradas.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key=f"{filtro_key}_download_no_encontradas",
        )

    if not df_match_cliente_sin_folio.empty:
        with st.expander("🟡 Facturas validadas solo por cliente (sin folio encontrado)", expanded=False):
            st.dataframe(df_match_cliente_sin_folio, use_container_width=True, hide_index=True)
...
def render_seguimiento_cobranza_tab_gerente(usuario_actual: str | None):
    ...
    if usuario_actual == "SChava":
        ...
        if com_hoy.empty:
            ...
            hora_prev = prev_dt.dt.strftime("%H:%M:%S")

            mask_prev_ok = prev_dt.notna()
            mask_hoy = mask_prev_ok & delta_days.eq(0)
            mask_ayer = mask_prev_ok & delta_days.eq(1)
            mask_antier = mask_prev_ok & delta_days.eq(2)
            mask_mas = mask_prev_ok & delta_days.gt(2)

            ultima_txt.loc[mask_hoy] = "Hoy " + hora_prev.loc[mask_hoy].fillna("")
            ultima_txt.loc[mask_ayer] = "Ayer " + hora_prev.loc[mask_ayer].fillna("")
            ...
            export_df["Última vez cobrado"] = ultima_txt.fillna("")
    ...
    row_sel_multi = st.multiselect(
        ...
        key=f"ger_seg_rows_multi_{cliente_sel}",
...
def render_salida_neta_tab():
    ...
    with st.expander("📤 Carga y procesamiento de archivos", expanded=True):
        endpoint_actual_admintotal = str(st.secrets.get("ADMINTOTAL_PRODUCTOS_ALMACEN_ENDPOINT", "")).strip()
        params_actuales_admintotal = _admintotal_productos_params()
        with st.expander("🔎 Fuente AdminTotal que usará el botón", expanded=False):
            if endpoint_actual_admintotal:
                st.success("El botón consultará el endpoint configurado explícitamente en `ADMINTOTAL_PRODUCTOS_ALMACEN_ENDPOINT`.")
                st.code(endpoint_actual_admintotal, language=None)
            else:
                st.warning(
                    "No está configurado `ADMINTOTAL_PRODUCTOS_ALMACEN_ENDPOINT`; el botón de actualizar intentará rutas API comunes, "
                    "pero la comparación rápida exige configurar la ruta exacta para no quedarse cargando."
                )
            st.caption("Parámetros que se enviarán para replicar Productos en 001 Matriz / Vigentes:")
            st.json(params_actuales_admintotal)
            st.caption("Al recibir XLSX/CSV, el sistema valida que la columna I se llame `Disponible` antes de escribir ROTACIONES.")
        ...
        with st.form("salida_neta_existencias_form", clear_on_submit=False):
            ...
            actualizar_existencias_admintotal = st.form_submit_button("🔄 Actualizar existencias desde AdminTotal")
            comparar_existencias_admintotal_manual = st.form_submit_button("🧪 Comparar AdminTotal vs Excel manual")
    ...
    def _normalizar_columna(col_name: str) -> str:
        col = str(col_name or "").strip().lower()
        col = unicodedata.normalize("NFKD", col).encode("ascii", "ignore").decode("ascii")
        return re.sub(r"\s+", " ", col)

    def _leer_existencias_excel(uploaded_file) -> pd.DataFrame:
        xls = pd.ExcelFile(uploaded_file)
        hoja = "Auxiliar de inventario" if "Auxiliar de inventario" in xls.sheet_names else xls.sheet_names[0]
        requeridas_norm = {"codigo", "disponible"}
        # Archivo de existencias: encabezados en fila 1 (header=0 en pandas).
        df_try = pd.read_excel(uploaded_file, sheet_name=hoja, header=0)
        cols_norm = {_normalizar_columna(c): c for c in df_try.columns}
        if requeridas_norm.issubset(set(cols_norm.keys())):
            return df_try.rename(columns={
                cols_norm["codigo"]: "Código",
                cols_norm["disponible"]: "Disponible",
            })
        cols_preview = ", ".join(str(c) for c in df_try.columns)
        raise ValueError(
            "El archivo de existencias no contiene las columnas requeridas: Código, Disponible. "
            f"Columnas detectadas: {cols_preview}"
        )
    ...
    def _ws_batch_update_safe(worksheet, data_ranges: list[dict]):
        """Batch update (una sola escritura API) con reintentos."""
    ...
    def _comparar_existencias_admin_manual(df_admin: pd.DataFrame, df_manual: pd.DataFrame, tolerancia: float = 0.000001):
        """Compara existencias agregadas por Código entre AdminTotal y el Excel manual."""
        def _normalizar(df: pd.DataFrame, col_valor: str) -> pd.DataFrame:
            tmp = df[["Código", "Disponible"]].copy()
            tmp["Código"] = tmp["Código"].astype(str).str.strip()
            tmp = tmp[tmp["Código"] != ""]
            tmp["Disponible"] = pd.to_numeric(tmp["Disponible"], errors="coerce").fillna(0)
            return tmp.groupby("Código", as_index=False)["Disponible"].sum().rename(columns={"Disponible": col_valor})

        admin_norm = _normalizar(df_admin, "Disponible_AdminTotal")
        manual_norm = _normalizar(df_manual, "Disponible_Manual")
        comparacion = admin_norm.merge(manual_norm, on="Código", how="outer")
        comparacion["Disponible_AdminTotal"] = comparacion["Disponible_AdminTotal"].fillna(0)
        comparacion["Disponible_Manual"] = comparacion["Disponible_Manual"].fillna(0)
        comparacion["Diferencia"] = comparacion["Disponible_AdminTotal"] - comparacion["Disponible_Manual"]
        comparacion["Coincide"] = comparacion["Diferencia"].abs() <= tolerancia
        diferencias = comparacion[~comparacion["Coincide"]].sort_values(["Diferencia", "Código"], ascending=[False, True]).reset_index(drop=True)
        resumen = {
            "productos_admin": len(admin_norm),
            "productos_manual": len(manual_norm),
            "productos_comparados": len(comparacion),
            "diferencias": len(diferencias),
            "diferencia_total": float(comparacion["Diferencia"].sum()) if not comparacion.empty else 0.0,
        }
        return resumen, comparacion, diferencias
    ...
    if actualizar_existencias_admintotal:
        try:
            with st.spinner("Consultando AdminTotal y actualizando existencias en ROTACIONES..."):
                resultado_admintotal = consultar_existencias_admintotal_productos()
                if not resultado_admintotal.get("success"):
                    raise Exception(resultado_admintotal.get("error", "Error desconocido al consultar AdminTotal."))
                df_existencias = resultado_admintotal.get("df", pd.DataFrame())
                if not isinstance(df_existencias, pd.DataFrame) or df_existencias.empty:
                    raise Exception("AdminTotal no devolvió productos con existencias.")
                df_existencias = df_existencias[["Código", "Disponible"]].copy()
                st.session_state["salida_neta_existencias_df"] = df_existencias
                cat_existencias_df = _actualizar_existencias_catalogo(df_existencias)
            st.success("✅ Existencias actualizadas desde AdminTotal usando Disponible (columna I del export de productos).")
            st.caption(f"Productos recibidos de AdminTotal: {len(df_existencias):,}")
            if resultado_admintotal.get("source_url"):
                st.caption(f"Fuente AdminTotal: {resultado_admintotal.get('source_url')}")
            metadata = resultado_admintotal.get("metadata") or df_existencias.attrs.get("admintotal_meta", {})
            if metadata:
                with st.expander("🔎 Evidencia de la consulta AdminTotal", expanded=True):
                    st.write(f"**Formato recibido:** {metadata.get('formato', 'No detectado')}")
                    st.write(f"**URL consultada:** {metadata.get('url_consultada', resultado_admintotal.get('source_url', ''))}")
                    st.write(f"**Columna usada para código:** {metadata.get('columna_codigo', '')}")
                    st.write(
                        "**Columna usada para existencias:** "
                        f"{metadata.get('columna_disponible', '')} "
                        f"({metadata.get('columna_disponible_excel', 'campo Disponible')})"
                    )
                    if metadata.get("hoja"):
                        st.write(f"**Hoja / fila encabezado:** {metadata.get('hoja')} / fila {metadata.get('fila_encabezado')}")
                    columnas_detectadas = metadata.get("columnas_detectadas") or []
                    if columnas_detectadas:
                        st.caption("Primeras columnas detectadas:")
                        st.code(", ".join(columnas_detectadas[:12]), language=None)
                    st.caption("Muestra de productos que se usaron para actualizar ROTACIONES:")
                    st.dataframe(df_existencias.head(10), use_container_width=True, hide_index=True)
            if isinstance(cat_existencias_df, pd.DataFrame) and not cat_existencias_df.empty:
                st.caption(f"Productos de catálogo revisados: {len(cat_existencias_df):,}")
        except Exception as e:
            st.error(f"❌ Error al actualizar existencias desde AdminTotal: {e}")

    if comparar_existencias_admintotal_manual:
        if not existencias_file:
            st.warning("⚠️ Sube el Excel manual de existencias para compararlo contra AdminTotal.")
        else:
            try:
                with st.spinner("Leyendo Excel manual y consultando AdminTotal para comparar existencias (máx. 35 segundos)..."):
                    df_manual = _leer_existencias_excel(existencias_file)[["Código", "Disponible"]].copy()
                    resultado_admintotal = consultar_existencias_admintotal_productos(
                        request_timeout=12,
                        total_timeout=35,
                        max_pages=10,
                        require_configured_endpoint=True,
                    )
                    if not resultado_admintotal.get("success"):
                        raise Exception(resultado_admintotal.get("error", "Error desconocido al consultar AdminTotal."))
                    df_admin = resultado_admintotal.get("df", pd.DataFrame())
                    if not isinstance(df_admin, pd.DataFrame) or df_admin.empty:
                        raise Exception("AdminTotal no devolvió productos con existencias para comparar.")
                    df_admin = df_admin[["Código", "Disponible"]].copy()
                    resumen_cmp, comparacion_cmp, diferencias_cmp = _comparar_existencias_admin_manual(df_admin, df_manual)
                    st.session_state["salida_neta_comparacion_existencias"] = {
                        "resumen": resumen_cmp,
                        "comparacion": comparacion_cmp,
                        "diferencias": diferencias_cmp,
                    }

                st.markdown("### 🧪 Comparación AdminTotal vs Excel manual")
                col_admin_cmp, col_manual_cmp, col_diff_cmp = st.columns(3)
                col_admin_cmp.metric("Productos AdminTotal", f"{resumen_cmp['productos_admin']:,}")
                col_manual_cmp.metric("Productos Excel manual", f"{resumen_cmp['productos_manual']:,}")
                col_diff_cmp.metric("Diferencias", f"{resumen_cmp['diferencias']:,}")
                st.caption(f"Diferencia total AdminTotal - Manual: {resumen_cmp['diferencia_total']:,.6g}")
                if resultado_admintotal.get("source_url"):
                    st.caption(f"Fuente AdminTotal comparada: {resultado_admintotal.get('source_url')}")

                if diferencias_cmp.empty:
                    st.success("✅ Perfecto: AdminTotal y el Excel manual coinciden en Disponible por Código. Ya puedes usar el botón de AdminTotal con confianza.")
                else:
                    st.warning("⚠️ Hay diferencias entre AdminTotal y el Excel manual. Revisa la tabla antes de actualizar ROTACIONES.")
                    st.dataframe(diferencias_cmp.head(500), use_container_width=True, hide_index=True)
            except Exception as e:
                st.error(f"❌ Error al comparar existencias AdminTotal vs Excel manual: {e}")
        ...
        if not entradas_file or not salidas_file:
            ...
            try:
                ...
                with st.spinner("Procesando archivos..."):
                    progress.progress(15, text="Leyendo archivos Excel...")
                    ...
                    progress.progress(40, text="Filtrando tipos excluidos...")
                    ...
                    progress.progress(60, text="Convirtiendo columnas numéricas y agrupando por Código...")
                    ...
                    entr_agg = (
                        ...
                        .agg({"Entrada": "sum", "Descripción": "first"})
                        .rename(columns={"Entrada": "Entrada_Total", "Descripción": "Descripción_Entrada"})
                    )
                    sal_agg = (
                        df_salidas.groupby("Código", as_index=False)
                        .agg({"Salida": "sum", "Descripción": "first"})
                        .rename(columns={"Salida": "Salida_Total", "Descripción": "Descripción_Salida"})
                    )

                    progress.progress(80, text="Uniendo información de Entradas y Salidas...")
    ...
    if catalogo_df.empty:
        try:
            catalogotd = st.secrets.get("catalogotd", "1CzJm9Goqs6SoeHrJkn76UQ7ofFIYmGpavG-5feqaAoA")
            ws_rot = gspread_client.open_by_key(catalogotd).worksheet("ROTACIONES")
            catalogo_df = pd.DataFrame(ws_rot.get_all_records())
        except Exception as e:
    ...
    if isinstance(catalogo_df, pd.DataFrame) and not catalogo_df.empty:
            ...
            col_map_norm = {_norm_col_name(c): c for c in cat.columns}

            def _pick_col(*aliases: str) -> str | None:
                for a in aliases:
                    found = col_map_norm.get(_norm_col_name(a))
                    if found:
                        return found
                return None

            col_modelo = _pick_col("Modelo", "Codigo", "Código")
            col_desc = _pick_col("Descripción", "Descripcion")
...
if "organizador" in tab_map:
    with tab_map["organizador"]:
        ...
        with sub_map["check_facturas"]:
            ...
            st.markdown("### 🤖 Carga desde AdminTotal")
            ahora_auto = now_cdmx()
            fecha_hasta_auto = ahora_auto.date()
            fecha_desde_auto = fecha_hasta_auto - timedelta(days=ADMINTOTAL_CHECK_FACTURAS_DIAS)
            hora_auto = ahora_auto.replace(
                hour=ADMINTOTAL_CHECK_FACTURAS_HORA[0],
                minute=ADMINTOTAL_CHECK_FACTURAS_HORA[1],
                second=0,
                microsecond=0,
            )
            st.caption(
                "La carga automática se dispara al abrir esta subpestaña después de las "
                f"{ADMINTOTAL_CHECK_FACTURAS_HORA[0]:02d}:{ADMINTOTAL_CHECK_FACTURAS_HORA[1]:02d}, "
                f"consulta AdminTotal del {fecha_desde_auto:%Y-%m-%d} al {fecha_hasta_auto:%Y-%m-%d} "
                "y aplica el mismo análisis de la carga manual: últimas 72 horas."
            )

            auto_fecha_key = "organizador_check_facturas_admintotal_auto_fecha"
            ejecutar_auto_admintotal = (
                ahora_auto >= hora_auto
                and st.session_state.get(auto_fecha_key) != fecha_hasta_auto.isoformat()
            )
            ejecutar_manual_admintotal = st.button(
                "🤖 Cargar últimos 4 días desde AdminTotal y analizar",
                key="organizador_check_facturas_admintotal_manual",
            )

            if ejecutar_auto_admintotal or ejecutar_manual_admintotal:
                try:
                    with st.spinner("Consultando facturas de AdminTotal y ejecutando check..."):
                        params_admintotal = {
                            "desde": fecha_desde_auto.strftime("%Y-%m-%d"),
                            "hasta": fecha_hasta_auto.strftime("%Y-%m-%d"),
                        }
                        resultado_admintotal = consultar_facturas_admintotal(params_admintotal)
                        if not resultado_admintotal.get("success"):
                            st.error(
                                "❌ Error al consultar facturas de AdminTotal: "
                                f"{resultado_admintotal.get('error', 'Error desconocido')}"
                            )
                            if resultado_admintotal.get("status_code") is not None:
                                st.write(f"**Status code:** {resultado_admintotal.get('status_code')}")
                        else:
                            df_admintotal = resultado_admintotal.get(
                                "df",
                                pd.DataFrame(columns=FACTURAS_FALTANTES_COLUMNAS),
                            )
                            resultado_check_auto = analizar_facturas_check_desde_df(
                                df_admintotal,
                                ventana_horas=ADMINTOTAL_CHECK_FACTURAS_VENTANA_ANALISIS_HORAS,
                            )
                            firma_auto = (
                                f"admintotal|{fecha_desde_auto:%Y-%m-%d}|{fecha_hasta_auto:%Y-%m-%d}|"
                                f"{len(df_admintotal)}|{hashlib.md5(df_admintotal.to_csv(index=False).encode('utf-8')).hexdigest()}"
                            )
                            st.session_state["organizador_check_facturas_admintotal_resultado"] = resultado_check_auto
                            st.session_state["organizador_check_facturas_admintotal_firma"] = firma_auto
                            if ejecutar_auto_admintotal:
                                st.session_state[auto_fecha_key] = fecha_hasta_auto.isoformat()
                            st.success("✅ Facturas de AdminTotal cargadas y analizadas correctamente.")
                except Exception as e:
                    st.error(f"❌ Error ejecutando check automático de AdminTotal: {e}")

            resultado_auto_guardado = st.session_state.get("organizador_check_facturas_admintotal_resultado")
            firma_auto_guardada = st.session_state.get("organizador_check_facturas_admintotal_firma")
            if resultado_auto_guardado and firma_auto_guardada:
                with st.expander("📊 Último resultado desde AdminTotal", expanded=True):
                    mostrar_resultado_check_facturas(
                        resultado_auto_guardado,
                        firma_guardado=firma_auto_guardada,
                        filtro_key="organizador_check_facturas_admintotal_filtro_vendedor",
                        guardado_sig_key="organizador_check_facturas_admintotal_guardado_sig",
                    )

            st.divider()
            st.markdown("### 📤 Carga manual por archivo")
