...
def get_cobranza_worksheet(nombre_hoja: str):
    spreadsheet_id = st.secrets["gsheets"].get("spreadsheet_id", SPREADSHEET_ID_MAIN)
    return gspread_client.open_by_key(spreadsheet_id).worksheet(nombre_hoja)
...
def cobranza_upsert_rows_by_key(ws, df: pd.DataFrame, key_cols: list[str], update_cols: list[str]):
    ...
    for _, r in df.iterrows():
        ...
        if key in idx:
            ...
            ws.update(f"{gspread.utils.rowcol_to_a1(row_num,1)}:{gspread.utils.rowcol_to_a1(row_num,len(headers))}", [row_vals])
        else:
...
def parse_antiguedad_cobranza_excel(file, mes: str) -> pd.DataFrame:
    ...
    for row in rows:
        ...
        i_sal = next((k for k, v in headers_idx.items() if "saldo" in v), None)
...
def get_cobranza_worksheets_safe():
    ...
    try:
        ...
        st.error(
            "❌ No fue posible abrir las hojas de Cobranza en Google Sheets. "
            "Verifica permisos/ID y vuelve a intentar."
        )
...
def render_cobranza_tab_gerente():
    ...
    c1, c2 = st.columns(2)
    with c1:
        year_sel = st.selectbox("Año", options=list(range(now_dt.year - 2, now_dt.year + 2)), index=2, key="ger_cob_year")
    with c2:
    ...
    if st.button("Generar y descargar Excel", key="ger_cob_excel"):
        ...
        if base_df.empty:
            ...
            with pd.ExcelWriter(bio, engine="xlsxwriter") as writer:
                ...
                ws.write(0, 0, f"Fecha De Generación: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}   Año: {year_i}   Mes: {calendar.month_name[month_i]}")
            bio.seek(0)
            st.download_button(
