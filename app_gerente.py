...
if "organizador" in tab_map:
    with tab_map["organizador"]:
        ...
        with sub[5]:
            ...
            feedback_guardado_key = "organizador_casos_feedback_guardado"
            feedback_guardado = st.session_state.get(feedback_guardado_key)
            if isinstance(feedback_guardado, dict):
                ts_feedback = float(feedback_guardado.get("ts", 0) or 0)
                if time.time() - ts_feedback <= 90:
                    comentario_feedback = str(feedback_guardado.get("comentario", "")).strip()
                    st.success(str(feedback_guardado.get("msg", "✅ Comentario_Gerente guardado correctamente.")))
                    if comentario_feedback:
                        st.caption(f"Comentario final guardado: {comentario_feedback}")
                else:
                    st.session_state.pop(feedback_guardado_key, None)
            ...
            if idx_caso is None or idx_caso not in df_casos_filtrado.index:
                ...
                if usar_nuevo_sistema:
                    ...
                    guardar_comentario = st.button("💾 Guardar comentario gerente", key=f"guardar_comentario_gerente_{idx_caso}")
                    ...
                    if guardar_comentario:
                        if not comentario_gerente.strip():
                            ...
                            try:
                                ...
                                st.session_state[feedback_guardado_key] = {
                                    "ts": time.time(),
                                    "msg": "✅ Comentario_Gerente guardado correctamente.",
                                    "comentario": comentario_gerente.strip(),
                                }
                                st.rerun()
                            except Exception as e:
