...
def _reportes_guia_es_guia_dhl_valida(guia) -> bool:
    """Una guía DHL válida solo puede traer números/espacios y exactamente 10 dígitos."""
    guia_txt = str(guia or "").strip()
    guia_sin_espacios = re.sub(r"\s+", "", guia_txt)
    return bool(re.fullmatch(r"[\d\s]+", guia_txt)) and len(guia_sin_espacios) == 10
...
def render_reportes_guia_tab():
    ...
    if not df_filtrado.empty:
        st.info(
            "📋 Modo DHL activo: se preseleccionan hasta 10 guías válidas más antiguas "
            "del resultado filtrado actual para copiarlas a DHL y después actualizar su estado."
        )
...
if "organizador" in tab_map:
    with tab_map["organizador"]:
        ...
        # Las subpestañas heredadas de agenda, pendientes, cotizaciones y checklist
        # se retiraron para evitar lecturas innecesarias a alejandro_data al abrir
        # Organizador. Las hojas relacionadas solo deben consultarse desde flujos
        # activos que todavía las necesiten.
        ...
        with sub_map["casos_especiales"]:
            ...
            primer_caso_filtrado = str(df_casos_filtrado.index[0]) if not df_casos_filtrado.empty else "__none__"
            selected_raw = st.session_state.get(select_key, primer_caso_filtrado)
            selected_norm = primer_caso_filtrado if selected_raw in (None, "", "__none__") else str(selected_raw)
            if selected_norm not in opciones_select:
                selected_norm = primer_caso_filtrado
