...
def get_alejandro_spreadsheet_id() -> str:
    """Obtiene el ID de Alejandro desde secrets o constante."""
    return st.secrets.get("gsheets", {}).get("spreadsheet_id_alejandro", SPREADSHEET_ID_ALEJANDRO)
...
def debug_alejandro_documento() -> dict:
    """Diagnóstico básico de alejandro_data usando gspread únicamente."""
    configured_id = get_alejandro_spreadsheet_id()
    out = {
        "spreadsheet_id": configured_id,
        "resolved_spreadsheet_id": configured_id,
        "drive_name": "N/A",
        "drive_mimeType": "N/A",
        "open_ok": False,
        "title": "",
        "url": f"https://docs.google.com/spreadsheets/d/{configured_id}",
        "worksheets": [],
        "missing_expected_sheets": [],
    }
    try:
        ss = gspread_client.open_by_key(configured_id)
        out["open_ok"] = True
        out["title"] = ss.title
