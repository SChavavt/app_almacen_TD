...
def consultar_existencias_admintotal_productos() -> dict:
    """
...
def render_salida_neta_tab():
    ...
    with st.expander("📤 Carga y procesamiento de archivos", expanded=True):
        ...
        with st.expander("🔎 Fuente AdminTotal que usará el botón", expanded=False):
            if endpoint_actual_admintotal:
                ...
                st.warning(
                    "No está configurado `ADMINTOTAL_PRODUCTOS_ALMACEN_ENDPOINT`; se intentarán rutas API comunes. "
                    "Para asegurar al 100% que sea el mismo export de la pantalla, configura aquí la ruta exacta del export de Productos por Almacén."
                )
    ...
    if comparar_existencias_admintotal_manual:
        if not existencias_file:
            ...
            try:
                with st.spinner("Leyendo Excel manual y consultando AdminTotal para comparar existencias..."):
