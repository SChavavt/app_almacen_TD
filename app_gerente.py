    ...
    if clientes_mes.empty:
        ...
        with st.form("ger_cob_form", clear_on_submit=False):
            ...
            fecha_pago = st.text_input(
                "Fecha de próximo pago (YYYY-MM-DD)",
                key="ger_cob_fecha_pago",
                placeholder="",
                help="Déjala vacía si aún no hay fecha compromiso. Si capturas fecha usa formato YYYY-MM-DD.",
            )
        ...
        if guardar_comentario:
            ...
            if not accion_code and not respuesta_code and not comentario.strip():
                ...
                if respuesta_code in respuestas_con_promesa and fecha_pago:
                    try:
                        fecha_proximo_pago = pd.to_datetime(fecha_pago, dayfirst=False, errors="raise").strftime("%Y-%m-%d")
                    except Exception:
                        fecha_invalida = True

                if fecha_invalida:
                    st.warning("⚠️ Fecha de próximo pago inválida. Usa formato YYYY-MM-DD.")
                else:
