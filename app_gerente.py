...
with tabs[0]:
    ...
    with sub[0]:
        ...
        if "Fecha_Proximo_Seguimiento" in cot.columns:
            ...
            cot_seguimiento = cot[est.str.contains("en seguimiento", na=False)].copy()
            ...
            cot_seg_venc = cot_seguimiento[cot_seguimiento["_fps"].notna() & (cot_seguimiento["_fps"].dt.date < hoy)].copy()
            cot_pend = cot_pend.sort_values("_fps", ascending=True)
            cot_venc = cot_venc.sort_values("_fps", ascending=True)
            cot_seguimiento = cot_seguimiento.sort_values("_fps", ascending=True)
            cot_seg_venc = cot_seg_venc.sort_values("_fps", ascending=True)
        else:
            cot_pend = cot.iloc[0:0]
            cot_venc = cot.iloc[0:0]
            cot_seguimiento = cot.iloc[0:0]
            cot_seg_venc = cot.iloc[0:0]
        ...
        k5.metric("💰 Cotiz. en seguimiento", len(cot_seguimiento))
        k6.metric("📈 Cumplimiento tareas de hoy", f"{tareas_hoy_pct}%")

        k7, k8, k9 = st.columns(3)
        k7.metric("💰 Cotizaciones pendientes", len(cot_pend))
        k8.metric("⚠️ Seg. de cotización vencido", len(cot_seg_venc))
        k9.metric("🧾 Checklist hoy", f"{pct}%")
