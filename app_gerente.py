...
if importlib.util.find_spec("googleapiclient.discovery"):
...
try:
    ...
    drive_client = build("drive", "v3", credentials=creds, cache_discovery=False) if build else None
...
def get_alejandro_spreadsheet_id() -> str:
    """Resuelve el ID usable de Alejandro (soporta shortcut de Drive si hay cliente disponible)."""
    configured_id = st.secrets.get("gsheets", {}).get("spreadsheet_id_alejandro", SPREADSHEET_ID_ALEJANDRO)

    # Fallback compatible: en entornos sin googleapiclient usamos el ID tal cual.
    if not drive_client:
        return configured_id

    meta = drive_client.files().get(
        fileId=configured_id,
    ...
    if mime == "application/vnd.google-apps.shortcut":
        ...
        target_mime = str(meta.get("shortcutDetails", {}).get("targetMimeType", ""))
        if target_id and target_mime == "application/vnd.google-apps.spreadsheet":
            return target_id
        raise Exception(
            f"El shortcut ({configured_id}) no apunta a un Google Sheet válido. target_mime={target_mime or 'N/A'}"
        )

    if mime != "application/vnd.google-apps.spreadsheet":
        raise Exception(
            "SPREADSHEET_ID_ALEJANDRO no es un Google Sheet nativo "
            f"(mimeType={mime or 'N/A'}). Convierte el archivo a Google Sheets y usa ese ID."
        )
...
def safe_append(nombre_hoja: str, row_dict: dict):
    ...
    try:
        ...
        if "not supported for this document" in msg.lower():
            raise Exception(
                "Google API rechazó la escritura: el documento no soporta append_row. "
                "Verifica que SPREADSHEET_ID_ALEJANDRO sea un Google Sheet nativo (no Excel) y que la hoja sea tipo GRID."
            ) from e
        raise


def debug_alejandro_documento() -> dict:
    """Diagnóstico del documento de Alejandro para detectar tipo de archivo/hojas."""
    out = {
        "spreadsheet_id": SPREADSHEET_ID_ALEJANDRO,
    ...
    try:
        ...
        if drive_client:
            ...
            out["drive_name"] = "N/A (googleapiclient no disponible)"
            out["drive_mimeType"] = "N/A"

        resolved_id = get_alejandro_spreadsheet_id()
...
with tabs[3]:
    ...
    with sub[5]:
        ...
        if st.button("Ejecutar diagnóstico", key="diag_alejandro_doc"):
            ...
            if not build:
                st.warning("Diagnóstico Drive extendido no disponible: falta dependencia googleapiclient en este entorno.")
            if ("not supported for this document" in err.lower()) or (
                mime and mime != "application/vnd.google-apps.spreadsheet" and mime != "application/vnd.google-apps.shortcut"
            ):
                st.error(
                    "El ID configurado no corresponde a un Google Sheet nativo (o shortcut válido a uno). "
                    "Convierte el archivo a Google Sheets y usa ese ID (o define gsheets.spreadsheet_id_alejandro en secrets)."
                )
