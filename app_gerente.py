...
def _admintotal_extraer_enlaces_exportacion(html: str, page_url: str) -> list[str]:
    ...
    for match in re.finditer(r'(?:href|data-url|data-href|action)=["\']([^"\']+)["\']', html_txt, flags=re.IGNORECASE):
        url = match.group(1)
        entorno = html_txt[max(0, match.start() - 300): match.end() + 300].lower()
        url_l = url.lower()
        if "export" in entorno or "excel" in entorno or "export" in url_l or ".xls" in url_l or ".csv" in url_l:
            enlaces.append(urllib.parse.urljoin(page_url, url))

    # Strings en JavaScript, por ejemplo onclick="location.href='...exportar...'".
    for match in re.finditer(r'["\']([^"\']*(?:export|excel|xlsx|xls|csv)[^"\']*)["\']', html_txt, flags=re.IGNORECASE):
        candidato = match.group(1).strip()
        if not candidato or candidato.startswith(("#", "javascript:")):
            continue
