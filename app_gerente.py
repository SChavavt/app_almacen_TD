...
def _send_mail_sendgrid_per_recipient(*, from_email: str, to_emails: list[str], subject: str, html_content: str, attachment_name: str, attachment_bytes: bytes, api_key: str):
    ...
    for email in to_emails:
        try:
            status_code, message_id = _send_mail_sendgrid(
            ...
            enviados.append({"email": email, "status": status_code, "message_id": message_id})
            ...
            if should_autosend and not sent_today:
                try:
                    ...
                    if enviados:
                        ...
                        enviados_txt = ", ".join([f"{x['email']} (id: {x['message_id'] or 'sin-id'})" for x in enviados])
                        st.success(f"📧 SendGrid aceptó el envío automático para: {enviados_txt}")
                        st.caption("Nota: aceptado por SendGrid (202) no garantiza entrega final; valida Activity/Suppressions en SendGrid.")
                    if fallidos:
            ...
            if st.button("📧 Enviar ahora por correo", use_container_width=True, key="ger_seg_cob_mail_hoy"):
                try:
                    ...
                    if enviados:
                        _COBRANZA_MAIL_LAST_SENT[stamp_key] = now_send_tz.isoformat()
                        enviados_txt = ", ".join([f"{x['email']} (id: {x['message_id'] or 'sin-id'})" for x in enviados])
                        st.success(f"✅ SendGrid aceptó el envío para: {enviados_txt}")
                        st.caption("Nota: aceptado por SendGrid (202) no garantiza entrega final; valida Activity/Suppressions en SendGrid.")
                    if fallidos:
                        for email, motivo in fallidos:
