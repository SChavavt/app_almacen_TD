...
def render_salida_neta_tab():
    ...
    def _actualizar_rotacion_catalogo(df_salida_neta: pd.DataFrame, fecha_desde: datetime):
        ...
        if col_objetivo not in headers:
            ...
            try:
                ...
                if insert_at <= len(headers):
                    rango_src = f"{gspread.utils.rowcol_to_a1(1, insert_at)}:{gspread.utils.rowcol_to_a1(ws.row_count, len(headers))}"
                    data_shift = ws.get(rango_src)
                    ws.update(
        ...
        cat_df[col_objetivo] = cat_df["Modelo"].map(salida_map).fillna(0)

        col_idx = headers.index(col_objetivo) + 1
        values_to_write = [[v] for v in cat_df[col_objetivo].tolist()]
        ws.update(f"{gspread.utils.rowcol_to_a1(2, col_idx)}:{gspread.utils.rowcol_to_a1(len(cat_df)+1, col_idx)}", values_to_write)
