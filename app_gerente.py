...
def render_salida_neta_tab():
    ...
    def _ws_update_range_safe(worksheet, a1_range: str, values_matrix):
        ...
        if hasattr(worksheet, "update"):
            try:
                ...
                raise
        # Evitar worksheet.range(...) porque dispara lectura adicional (quota READ).
        flat_values = [v for row in values_matrix for v in row]
        if len(flat_values) == 1 and ":" in a1_range and hasattr(worksheet, "update_cell"):
            a1_cell = a1_range.split(":", 1)[0]
            row_i, col_i = gspread.utils.a1_to_rowcol(a1_cell)
            return _retry_gspread_api_call(
                lambda: worksheet.update_cell(row_i, col_i, flat_values[0]),
                retries=5,
                base_delay=1.0,
            )
        raise RuntimeError("No se encontró método de escritura compatible para worksheet sin update().")
    ...
    def _ws_batch_update_safe(worksheet, data_ranges: list[dict]):
        ...
        try:
            ...
            # Compatibilidad con versiones de gspread: intentar payload alterno antes de fallback por rango.
            try:
                return _retry_gspread_api_call(
                    lambda: worksheet.batch_update(
                        {"data": data_ranges, "valueInputOption": "USER_ENTERED"}
                    ),
                    retries=5,
                    base_delay=1.0,
                )
            except Exception:
                if "not enough values to unpack" in str(exc) or "RESOURCE_EXHAUSTED" in str(exc) or "Quota exceeded" in str(exc):
                    for item in data_ranges:
                        _ws_update_range_safe(worksheet, item["range"], item["values"])
                    return None
                raise
                ...
                if guardar_captura:
                    try:
                        ...
                        if not col_comprar:
                            ...
                            # Estrategia tipo Tránsito: una sola escritura de rango continuo para evitar cuotas/read extra.
                            valores_columna = pd.to_numeric(cat[proveedor_captura_col], errors="coerce") if proveedor_captura_col in cat.columns else pd.Series([None] * len(cat))
                            valores_columna = valores_columna.astype(object)
                            for idx, (_, row) in enumerate(edited_df.iterrows()):
                                sheet_row = int(sheet_rows_editor.iloc[idx])
                                pos = sheet_row - 2
                                if pos < 0 or pos >= len(valores_columna):
                                    continue
                                cantidad_val = pd.to_numeric(row.get(nombre_col_cantidad), errors="coerce")
                                if pd.isna(cantidad_val):
                                    valores_columna.iloc[pos] = ""
                                else:
                                    valores_columna.iloc[pos] = int(cantidad_val) if float(cantidad_val).is_integer() else float(cantidad_val)

                            rango_col = f"{gspread.utils.rowcol_to_a1(2, col_proveedor_qty_ws)}:{gspread.utils.rowcol_to_a1(len(cat)+1, col_proveedor_qty_ws)}"
                            valores_matriz = [["" if pd.isna(v) else v] for v in valores_columna.tolist()]
                            data_ranges = [{"range": rango_col, "values": valores_matriz}]
                            if data_ranges:
                                with st.spinner("Guardando captura manual en ROTACIONES (1 sola escritura)..."):
