    ...
    with tab_map["organizador"]:
        ...
        with sub[5]:
            ...
            if feedback_ok:
                ...
                col_feedback_a, col_feedback_b = st.columns(2)
                with col_feedback_a:
                    if st.button("🧹 Limpiar mensaje", key="organizador_casos_limpiar_feedback"):
                        st.session_state.pop("organizador_casos_feedback_ok", None)
                        st.session_state.pop("organizador_casos_feedback_comment", None)
                        st.rerun()
                with col_feedback_b:
                    if st.button("🔄 Actualizar vista", key="organizador_casos_refrescar_vista"):
                        st.rerun()
