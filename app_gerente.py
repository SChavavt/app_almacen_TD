    ...
    if clientes_mes.empty:
        ...
        with st.form("ger_cob_form", clear_on_submit=False):
            ...
            if st.session_state.get("ger_cob_prefill_ctx") != prefill_ctx:
                ...
                for k in [
                    ...
                    "ger_cob_fecha_pago", "ger_cob_fecha_modo", "ger_cob_fecha_picker",
                ...
                st.session_state["ger_cob_fecha_pago"] = fecha_pago_existente_txt
                st.session_state["ger_cob_fecha_modo"] = "CAPTURAR" if fecha_pago_existente_txt else ""
            ...
            fecha_modo = st.selectbox(
                "Fecha de próximo pago",
                options=["", "CAPTURAR"],
                format_func=lambda v: "Sin fecha" if v == "" else "Seleccionar fecha",
                key="ger_cob_fecha_modo",
            )
            fecha_pago = ""
            if fecha_modo == "CAPTURAR":
                fecha_pago_dt = st.date_input(
                    "Selecciona fecha de próximo pago",
                    key="ger_cob_fecha_picker",
                    format="DD/MM/YYYY",
                )
                if fecha_pago_dt:
                    fecha_pago = pd.to_datetime(fecha_pago_dt).strftime("%Y-%m-%d")
        ...
        if guardar_comentario:
            ...
            if not accion_code and not respuesta_code and not comentario.strip():
                ...
                estatus_guardado = "LIQUIDADO" if es_pagado else estatus_seguimiento
