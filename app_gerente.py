...
def _reportes_guia_sheet_id() -> str:
    ...
    return _extract_sheet_id(st.secrets.get("reportes_sheet_id", ""))
...
def get_reportes_guia_worksheet():
    ...
    if not sheet_id:
        raise ValueError("Falta configurar el secret reportes_sheet_id.")
    return _retry_gspread_api_call(
...
def render_reportes_guia_tab():
    ...
    recibido_norm = df_all["RECIBIDO POR"].astype(str).str.strip().str.upper()
    df_view = df_all[recibido_norm != "ENTREGADO"].copy()
