...
def _admintotal_es_url_exportacion_relevante(url: str) -> bool:
    """Valida URLs candidatas y descarta HTML o enlaces globales ajenos al modal."""
    url_txt = str(url or "").strip()
    url_l = url_txt.lower()
    if not url_l or url_l.startswith(("#", "javascript:")):
        return False
    # Si el regex capturó un fragmento HTML del modal/notificación, no es una URL.
    if any(ch in url_txt for ch in ("<", ">", "\n", "\r", "\t")) or re.search(r"\s", url_txt):
        return False
    if len(url_txt) > 300:
        return False

    parsed = urlparse(url_txt)
    es_absoluta = parsed.scheme in {"http", "https"} and bool(parsed.netloc)
    es_relativa = url_txt.startswith(("/", "./", "../", "?")) or re.match(r"^[A-Za-z0-9_./?=&%-]+$", url_txt)
    if not es_absoluta and not es_relativa:
        return False
...
def _admintotal_intentos_exportacion_productos(page_url: str, params: dict, html: str) -> list[dict]:
    ...
    # Primero se prueba POST contra la página actual: en AdminTotal los renglones del modal
    # suelen disparar una acción con los filtros actuales, no una ruta REST visible.
    acciones_opcion_1 = [
        {"exportar": "1"},
        {"export": "1"},
        {"excel": "1"},
        ...
        {"reporte": "productos_almacen", "opcion": "1"},
        {"tipo_reporte": "productos_almacen", "opcion": "1"},
    ]
...
def _admintotal_intento_a_texto(intento: dict) -> str:
    """Muestra intentos sin query larga ni fragmentos HTML accidentales."""
    method = intento.get("method", "GET")
    url = str(intento.get("url", ""))
    parsed = urlparse(url)
    ruta = parsed.path or url[:80]
    data = intento.get("data") or {}
    claves_accion = [k for k in data if k not in {"csrfmiddlewaretoken", "estado", "almacen", "q", "buscar", "search"}]
    detalle = f" ({', '.join(claves_accion[:3])})" if claves_accion else ""
    return f"{method} {ruta}{detalle}"
...
def consultar_productos_almacen_admin_admintotal(ruta_admin: str, params: dict | None = None) -> dict:
    ...
    try:
        ...
        for intento in intentos:
            ...
            intentos_probados.append(_admintotal_intento_a_texto(intento))
            if method == "POST":
                csrf_token = session.cookies.get("csrftoken") or _admintotal_csrf_token(session, pagina.text)
                headers = {"Referer": pagina.url}
                post_data = dict(intento.get("data") or {})
                if csrf_token:
                    headers["X-CSRFToken"] = csrf_token
                    post_data.setdefault("csrfmiddlewaretoken", csrf_token)
                respuesta = session.post(export_url, data=post_data, headers=headers, timeout=90)
            else:
...
PEDIDOS_COLUMNAS_MINIMAS = [
    ...
    "Terminal", "Referencia_Pago", "Monto_Venta_Terceros", "Plazo_Credito_Meses", "Frecuencia_Pago",
...
def formatear_fecha_factura_check_visual(valor) -> str:
    """Usa solo la columna Fecha y conserva su hora cuando ya viene incluida."""
    texto = str(valor or "").strip()
    if not texto:
        return ""
    ...
    fecha_trae_hora = _admintotal_fecha_tiene_hora(texto) or isinstance(valor, (pd.Timestamp, datetime))
    if not fecha_trae_hora:
        return fecha.strftime("%d/%m/%Y")

    if fecha.second or re.search(r"(?:T|\s)\d{1,2}:\d{2}:\d{2}", texto):
...
def _venta_terceros_parse_fecha_date(valor: str) -> date | None:
    """Convierte la fecha guardada a date para inicializar date_input."""
    texto = str(valor or "").strip()
    if not texto:
        return None
    fecha = pd.to_datetime(texto, errors="coerce", dayfirst=True)
...
VENTA_TERCEROS_CARD_PAYMENT_METHODS = ("Tarjeta de Débito", "Tarjeta de Crédito")
...
def _venta_terceros_safe_index(options, value: str | None) -> int:
    """Devuelve un índice seguro para selectbox aunque el valor no exista."""
...
def _venta_terceros_parse_monto(valor: str) -> float:
    """Convierte importes con símbolos, comas o espacios a número."""
    texto = str(valor or "").strip()
    if not texto:
        return 0.0
...
def _venta_terceros_formatear_monto(valor: float) -> str:
    """Muestra importes clave con moneda y separadores."""
    return f"${valor:,.2f}"
...
def _venta_terceros_render_upload_comprobantes(row: pd.Series):
    ...
    st.caption(
        "Carga comprobantes y captura el pago; el sistema agregará un registro automático "
        "al historial de cobros con la cantidad abonada."
    )
...
def _venta_terceros_render_credito(row: pd.Series):
    ...
    monto_venta_txt = _venta_terceros_valor(row, ["Monto_Venta_Terceros", "Monto_Venta", "Monto_Comprobante"], "0.00")
    anticipo_txt = _venta_terceros_valor(row, ["Anticipo_Credito", "Anticipo", "Anticipo_Venta_Terceros"], "0.00")
    restante_cobrar = _venta_terceros_parse_monto(monto_venta_txt) - _venta_terceros_parse_monto(anticipo_txt)

    col1, col2, col3 = st.columns(3)
    with col1:
        _venta_terceros_metric("💲 Monto de la venta", monto_venta_txt)
        _venta_terceros_metric("💵 Anticipo (opcional)", anticipo_txt)
    with col2:
...
def load_reportes_guia_from_gsheets() -> pd.DataFrame:
    ...
    for sheet_row, row_values in enumerate(values[1:], start=2):
        ...
        if any(row.get(col, "") for col in REPORTES_GUIA_COLUMNS):
...
def _reportes_guia_es_entregado(valor: str) -> bool:
    """Detecta ENTREGADO aun si fue capturado con letras repetidas por error."""
    valor_norm = normalizar(str(valor or "").strip())
    valor_colapsado = _reportes_guia_colapsar_letras_repetidas(valor_norm)
    return valor_norm == "ENTREGADO" or valor_colapsado == "ENTREGADO"
...
def _reportes_guia_recibido_filter_label(valor: str) -> str:
    ...
    if not valor_txt:
        return "PENDIENTES"
    valor_norm = normalizar(valor_txt)
    if not re.search(r"[a-z0-9]", valor_norm):
...
def render_reportes_guia_tab():
    ...
    for idx, col in enumerate(REPORTES_GUIA_FILTER_COLUMNS):
        ...
        with fcols[idx]:
            filtros[col] = st.multiselect(col, options=opciones, key=f"reportes_guia_filter_{col}")
    ...
    st.markdown("#### Actualización individual de RECIBIDO POR")
    if df_filtrado.empty:
        st.caption("No hay filas para actualizar con los filtros actuales.")
        return
    opciones_fila = [f"Fila {int(r['__sheet_row'])} — {r.get('GUIA', '')} — {r.get('NOMBRE', '')}" for _, r in df_filtrado.iterrows()]
    ...
    accion = st.radio("Nuevo valor", ["ENTREGADO", "HOY AL FINAL DEL DIA", "Fecha (MIERCOLES 10)"], horizontal=True, key="reportes_guia_accion")
    valor_final = accion
    if accion == "Fecha (MIERCOLES 10)":
        fecha_sel = st.date_input("Selecciona fecha", value=now_cdmx().date(), format="YYYY-MM-DD", key="reportes_guia_fecha_ind")
        valor_final = _reportes_guia_fecha_a_texto(fecha_sel)
        st.caption(f"Se escribirá: **{valor_final}**")
    if st.button("💾 Guardar cambio individual", key="reportes_guia_guardar_individual"):
        ok, fail = update_reportes_guia_recibido([sheet_row_sel], valor_final)
        st.success(f"✅ Se actualizó la fila {sheet_row_sel} con {valor_final}.")
        st.rerun()
