    ...
    if clientes_mes.empty:
        ...
        if not com_df.empty:
            com_mes_cliente = com_df[
                ...
                & (com_df.get("Folio", "").astype(str).isin(folios_sel_pre) if folios_sel_pre else False)
        ...
        with st.container(border=True):
            st.markdown("**Selección de folios para comentario en lote**")
            with st.form("ger_cob_filtros_form", clear_on_submit=False):
                st.multiselect(
                    "Folio",
                    options=folios_disponibles,
                    default=folios_sel_pre if folios_sel_pre else folios_disponibles[:1],
                    key="ger_cob_folios",
                    help="Folios activos del cliente según el filtro actual. Puedes seleccionar varios y aplicar con el botón.",
                )
                st.selectbox(
                    "Día",
        ...
        folios_sel = st.session_state.get("ger_cob_folios", folios_disponibles[:1])
        ...
        dia_sel = st.session_state.get("ger_cob_dia", dia_default)
