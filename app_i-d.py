...
def assign_flow_numbers(entries_local, entries_foraneo, df_all: pd.DataFrame) -> None:
    ...
    active_foraneo_entries = [
        ...
        if not _is_cancelado_estado(e.get("estado", ""))
        and sanitize_text(e.get("completados_limpiado", "")).lower() != "sí"
    ]
    ...
    def assign(
        ...
        for entry in entries:
            ...
            if suppress_cancelled_number and sanitize_text(entry.get("completados_limpiado", "")).lower() == "sí":
                entry["numero"] = ""
                continue
