...
def apply_kiosk_fullscreen_css() -> None:
    ...
    st.markdown(
        ...
        iframe { display: block !important; border: 0 !important; margin: 0 !important; }
        </style>
        """,
...
if is_tv_wall_view:
    ...
    _, col_update_only, col_action_only = st.columns([0.62, 0.22, 0.16])
    ...
    if not df_local.empty:
        ...
        if not df_local_turno_reset.empty:
            df_local_turno_reset["_turno_local_norm"] = (
                df_local_turno_reset["Turno"].astype(str).str.strip().replace({"🌤️ Local Día": "☀️ Local Mañana"})
            )
            df_local_turno_reset["_turno_local_norm"] = df_local_turno_reset["_turno_local_norm"].replace("", "sin_turno")
            df_local_turno_reset["_fecha_local_norm"] = pd.to_datetime(
                df_local_turno_reset.get("Fecha_Entrega", ""), errors="coerce"
            ).dt.strftime("%Y-%m-%d")
            df_local_turno_reset["_fecha_local_norm"] = df_local_turno_reset["_fecha_local_norm"].fillna("sin_fecha")
            df_local_turno_reset["_sort_dt"] = df_local_turno_reset.apply(compute_sort_key, axis=1)
            df_local_turno_reset = df_local_turno_reset.sort_values(
                by=["_turno_local_norm", "_fecha_local_norm", "_sort_dt"], kind="mergesort"
            ).reset_index(drop=True)

            counters_by_bucket: dict[tuple[str, str], int] = {}
            for _, row in df_local_turno_reset.iterrows():
                turno = str(row.get("_turno_local_norm", "")).strip() or "sin_turno"
                fecha = str(row.get("_fecha_local_norm", "")).strip() or "sin_fecha"
                bucket = (turno, fecha)
                counters_by_bucket[bucket] = counters_by_bucket.get(bucket, 0) + 1
                numero = str(counters_by_bucket[bucket])
                row_key = _flow_row_key_from_row(row)
                for raw_key in (row_key, row.get("ID_Pedido", ""), row.get("Folio_Factura", "")):
                    key = raw_key if isinstance(raw_key, str) and raw_key.startswith("row:") else _flow_match_key(raw_key)
        ...
        if not df_local_otro.empty:
            map_local_fallback = build_map(df_local_otro.reset_index(drop=True), lambda idx: str(idx + 1))
            for key, numero in map_local_fallback.items():
                if key and key not in map_local:
                    map_local[key] = numero

    foraneo_map = build_map(df_foraneo, lambda idx: f"{idx + 1:02d}")
    return map_local, foraneo_map
...
def render_auto_list(
    ...
    compact_foraneo_overflow: bool = False,
):
    if not entries:
        st.info("No hay pedidos para mostrar.")
        return start_number
    ...
    for fallback_number, e in visible:
        completados_limpiado_text = sanitize_text(e.get("completados_limpiado", "")).lower()
        is_priority = "priori" in completados_limpiado_text
        is_mod_material_suppressed = bool(
            re.search(r"\bno\b", completados_limpiado_text)
        )
        is_mod_material = (
            "por material" in sanitize_text(e.get("tipo_modificacion", "")).lower()
            and not is_mod_material_suppressed
        )
        ...
        is_pantalla_f_foraneo_row = compact_foraneo_overflow and layout_user == "PANTALLAF" and mode == "foraneo"
        ...
        surtidor_style = surtidor_badge_style(surtidor)
        if is_pantalla_f_foraneo_row:
            surtidor_style = f"{surtidor_style};color:#000!important;"
        surtidor_html = (
            f"<span class='surtidor-tag' style=\"{surtidor_style}\">{surtidor_display}</span>"
            if surtidor
            else ""
        )
    ...
    is_compact_pantalla_f_foraneo_view = compact_foraneo_overflow and is_pantalla_f_foraneo_view
    if show_header and is_compact_pantalla_f_foraneo_view:
        compact_subtitle = f"{subtitle} · Mostrando {len(visible)}/{len(entries)}" if subtitle else f"Mostrando {len(visible)}/{len(entries)}"
        header_html = f"""
    <div class="board-title board-title-compact">
        <div class="board-title-main">{title}</div>
        <div class="board-sub board-sub-inline">{compact_subtitle}</div>
    </div>
    """
    else:
        header_html = (
    ...
    # PANTALLAL se muestra en una TV más chica que PANTALLAF. Usamos una
    # altura intermedia: compacta para no dejar huecos entre tableros, pero con
    # suficiente aire para que el iframe no recorte encabezados ni filas.
    row_height_px = 31 if is_pantalla_l_local_view else (52 if is_large_auto_list_view else 38)
    title_height_px = (
        34
        if show_header and is_compact_pantalla_f_foraneo_view
        else (88 if show_header and is_pantalla_l_local_view else (58 if show_header else 10))
    )
    default_min_content = 132 if is_pantalla_l_local_view else (140 if show_header else 100)
    min_content = min_content_height if min_content_height is not None else default_min_content
    safety_padding_px = 16 if is_pantalla_l_local_view else 24
    content_height = max(
    ...
    compact_board_padding = "0.28rem 0.38rem" if is_pantalla_l_local_view else "0.55rem 0.68rem"
    compact_title_margin = "0.18rem" if is_pantalla_l_local_view else "0.4rem"
    compact_head_padding = "0.12rem 0.16rem" if is_pantalla_l_local_view else "0.2rem 0.2rem"
    compact_cell_padding = "0.08rem 0.12rem" if is_pantalla_l_local_view else "0.14rem 0.16rem"
    compact_cell_line_height = "0.92rem" if is_pantalla_l_local_view else "1.02rem"

    colgroup_html = (
        "<col style='width:3ch'><col style='width:16ch'><col style='width:4ch'><col style='width:3ch'>"
        if is_compact_pantalla_f_foraneo_view
        else (
            "<col style='width:8%'><col style='width:49%'><col style='width:19%'><col style='width:24%'>"
            if is_pantalla_f_foraneo_view
            else (
    ...
    surtidor_header = "Sur" if is_compact_pantalla_f_foraneo_view else "Surtidor"
    estado_header = "Es" if is_compact_pantalla_f_foraneo_view else "Estado"

    html = f"""
    <style>
    .board-col{{flex:1;background:rgba(18,18,20,0.92);border-radius:0.9rem;padding:{compact_board_padding};box-shadow:0 2px 12px rgba(0,0,0,0.25);height:100%;font-family:"Source Sans Pro", sans-serif;}}
    .board-title{{display:flex;justify-content:space-between;align-items:center;gap:0.6rem;margin-bottom:{compact_title_margin};font-weight:600;font-size:{compact_title_size};color:#fff;letter-spacing:0.01em;}}
    .board-title-compact{{flex-direction:column;align-items:flex-start;gap:0.04rem;margin-bottom:0.12rem;line-height:0.95;}}
    .board-title-main{{max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
    .board-sub{{font-size:{compact_sub_size};opacity:0.8;font-weight:500;}}
    .board-sub-inline{{max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
    .board-table{{width:100%;border-collapse:collapse;table-layout:fixed;}}
    .board-head th{{font-size:{compact_head_size};letter-spacing:0.02em;text-transform:uppercase;color:#d7e6ff;font-weight:700;padding:{compact_head_padding};border-bottom:1px solid rgba(140,180,255,0.38);text-align:left;background:rgba(65,105,180,0.16);white-space:nowrap;}}
    ...
    .board-row td{{padding:{compact_cell_padding};vertical-align:middle;font-size:{compact_td_size};color:#fff;line-height:{compact_cell_line_height};}}
    ...
    .board-surtidor .surtidor-tag{{display:inline-block;margin-left:0;padding:0.09rem 0.4rem;font-size:{compact_surtidor_size};font-weight:800;{compact_surtidor_color_css}}}
    ...
    <div class="{scroll_class}">
        <table class="board-table">
            ...
            <thead class="board-head">
                <tr>
                    <th>#</th>
                    <th>{cliente_header}</th>
                    {f"<th>{surtidor_header}</th>" if mode in {"foraneo", "local"} else ""}
                    <th>{estado_header}</th>
                </tr>
            </thead>
            {''.join(rows_html)}
...
def dedupe_entries_preserve_order(entries):
    ...
    for entry in entries:
        key_parts = []
        id_pedido = sanitize_text(entry.get("id_pedido", ""))
        folio = sanitize_text(entry.get("folio", ""))
        cliente_nombre = sanitize_text(entry.get("cliente_nombre", "")) or sanitize_text(entry.get("cliente", ""))
        fecha = sanitize_text(entry.get("fecha", ""))
        estado = sanitize_text(entry.get("estado", ""))

        if id_pedido:
            key_parts.append(f"id:{id_pedido.lower()}")
        if folio:
            key_parts.append(f"folio:{folio.lower()}")
        if cliente_nombre:
            key_parts.append(f"cliente:{cliente_nombre.lower()}")
        if fecha:
            key_parts.append(f"fecha:{fecha.lower()}")
        if estado:
            key_parts.append(f"estado:{estado.lower()}")
    ...
    if not frames:
        ...
        if df_rep.empty:
            ...
            if periodo == "Día":
                ...
                filtro_firma = f"Semana|{semana_sel}"
            else:
                ...
                filtro_firma = f"Mes|{mes_sel}"

            filtro_anterior = st.session_state.get("rep_surt_filtro_firma")
            if filtro_anterior is None:
                st.session_state["rep_surt_filtro_firma"] = filtro_firma
            elif filtro_anterior != filtro_firma:
                st.session_state["rep_surt_filtro_firma"] = filtro_firma
                load_historicos_from_gsheets.clear()
                load_data_from_gsheets.clear()
                st.rerun()
            ...
            if total == 0:
                ...
                resumen_surtidores["Pedidos"] = resumen_surtidores["Pedidos"].fillna(0).astype(int)
                ...
                st.markdown("##### 📄 Lista 2: Detalle de pedidos filtrados")
...
if selected_tab_key == "surtidores":
    ...
    with st.form("surtidores_asignacion_form"):
        ...
        st.markdown(
            """
            <style>
            div[data-testid="stRadio"] div[role="radiogroup"][aria-label="Surtidor"] > label[data-baseweb="radio"]{
                display:inline-flex;align-items:center;gap:0.18rem;
            }
            div[data-testid="stRadio"] div[role="radiogroup"][aria-label="Surtidor"] > label[data-baseweb="radio"] p{
                border-radius: 0.7rem;
                padding: 0.08rem 0.42rem;
                font-weight: 800 !important;
                color: #111827 !important;
            }
            div[data-testid="stRadio"] div[role="radiogroup"][aria-label="Surtidor"] > label[data-baseweb="radio"]:nth-child(1) p{background:#ff5a5f;}
            div[data-testid="stRadio"] div[role="radiogroup"][aria-label="Surtidor"] > label[data-baseweb="radio"]:nth-child(2) p{background:#4da3ff;}
            div[data-testid="stRadio"] div[role="radiogroup"][aria-label="Surtidor"] > label[data-baseweb="radio"]:nth-child(3) p{background:#58d68d;}
...
if selected_tab_key == "auditores":
    ...
    for e in auto_foraneo_entries:
        ...
        if not num_txt:
            continue
        keys = {
            build_surtidor_key(e),
            _flow_row_key_from_entry(e),
            _flow_match_key(e.get("id_pedido", "")),
            _flow_match_key(e.get("folio", "")),
        }
        for key in keys:
            if key:
                foraneo_num_map[key] = num_txt
    if not df_all.empty:
    ...
    if not (foraneo_entries or local_entries):
        ...
        for e in (foraneo_entries + local_entries):
            ...
            if tipo_envio == "🚚 Pedido Foráneo":
                numero_foraneo_txt = (
                    foraneo_num_map.get(key, "")
                    or foraneo_num_map.get(_flow_row_key_from_entry(e), "")
                    or foraneo_num_map.get(_flow_match_key(e.get("id_pedido", "")), "")
                    or foraneo_num_map.get(_flow_match_key(e.get("folio", "")), "")
                )
    ...
    with st.expander(
        ...
        if df_facturas_faltantes.empty:
            ...
            if vendedor_col:
                ...
                logged_vendor_en_facturas = False
                if logged_vendor_facturas:
                    logged_vendor_en_facturas = any(
                        _vendedor_names_match(opt, logged_vendor_facturas)
                        for opt in vendedores_disponibles
                    )
                ...
                with filtros_cols[0]:
                    ...
                    if logged_vendor_facturas and not logged_vendor_en_facturas:
                        st.success(
                            "✅ Al parecer no tienes facturas faltantes por enviar."
                        )
