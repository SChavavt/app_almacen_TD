...
@st.cache_resource
def get_cobranza_spreadsheet():
    creds_info = json.loads(st.secrets["gsheets"]["google_credentials"])
...
def ensure_headers(worksheet, expected_headers: list[str]):
    header_row = worksheet.row_values(1)
    if header_row != expected_headers:
        worksheet.update("1:1", [expected_headers])


def load_sheet_records_with_row_numbers(worksheet) -> list[dict]:
    data = worksheet.get_all_values()
    if not data:
        return []
    headers = data[0]
    ...
    for i, row in enumerate(data[1:], start=2):
        ...
        rec = {headers[j]: row_vals[j] for j in range(len(headers))}
        rec["__row_number__"] = i
        records.append(rec)
    return records


def upsert_rows_by_key(worksheet, df: pd.DataFrame, key_cols: list[str], update_cols: list[str]):
    if df.empty:
        return 0, 0
    records = load_sheet_records_with_row_numbers(worksheet)
    headers = worksheet.row_values(1)
    key_index = {
        tuple(sanitize_text(rec.get(k, "")) for k in key_cols): rec
        for rec in records
    }
    updated = 0
    appended = 0
    for _, row in df.iterrows():
        key = tuple(sanitize_text(row.get(k, "")) for k in key_cols)
        row_dict = {h: "" for h in headers}
        for col in headers:
            if col in df.columns:
                row_dict[col] = _clean_sheet_value(row.get(col, ""))
        if key in key_index:
            rec = key_index[key]
            row_num = rec["__row_number__"]
            existing_vals = [rec.get(h, "") for h in headers]
            for col in set(key_cols + update_cols):
                if col in headers:
                    existing_vals[headers.index(col)] = row_dict.get(col, "")
            start = gspread.utils.rowcol_to_a1(row_num, 1)
            end = gspread.utils.rowcol_to_a1(row_num, len(headers))
            worksheet.update(f"{start}:{end}", [existing_vals])
            updated += 1
        else:
            append_vals = [row_dict.get(h, "") for h in headers]
            worksheet.append_row(append_vals, value_input_option="USER_ENTERED")
            appended += 1
    return updated, appended


def parse_reporte_excel(file, mes: str) -> pd.DataFrame:
    raw = pd.read_excel(file, header=None)
    header_idx = None
    for i in range(len(raw)):
        row_vals = [sanitize_text(v).lower() for v in raw.iloc[i].tolist()]
        if any("codigo" in v or "código" in v for v in row_vals):
            header_idx = i
            break
    if header_idx is None:
        raise ValueError("No se encontró encabezado con 'Código' en REPORTE.xlsx")

    df = pd.read_excel(file, header=header_idx)
    col_map = {}
    for c in df.columns:
        name = sanitize_text(c).lower().replace("ó", "o")
        if name == "codigo" or name == "código":
            col_map[c] = "Codigo"
        elif "razon" in name and "social" in name:
            col_map[c] = "Razon_Social"
        elif "no vencido" in name:
            col_map[c] = "No_Vencido"
        elif name == "saldo":
            col_map[c] = "Saldo"
        elif "vencido" in name:
            col_map[c] = "Vencido"
    df = df.rename(columns=col_map)
    for required in ["Codigo", "Razon_Social", "Saldo", "No_Vencido"]:
        if required not in df.columns:
            raise ValueError(f"Falta columna requerida en REPORTE: {required}")
    if "Vencido" not in df.columns:
        df["Vencido"] = 0.0

    df = df[["Codigo", "Razon_Social", "Saldo", "No_Vencido", "Vencido"]].copy()
    df["Codigo"] = df["Codigo"].apply(_normalize_code)
    df = df[df["Codigo"].astype(str).str.strip() != ""]
    for ncol in ["Saldo", "No_Vencido", "Vencido"]:
        df[ncol] = df[ncol].apply(_to_float)
    df["Mes"] = mes
    return df[["Mes", "Codigo", "Razon_Social", "Saldo", "No_Vencido", "Vencido"]]


def parse_antiguedad_excel(file, mes: str) -> pd.DataFrame:
    raw = pd.read_excel(file, header=None)
    ...
    out = []
    for row in rows:
        c0 = sanitize_text(row[0] if len(row) > 0 else "")
        c1 = sanitize_text(row[1] if len(row) > 1 else "")
        try:
            if c0 and float(str(c0).replace(",", "")) > 0 and c1:
                current_codigo = _normalize_code(c0)
                detail_idx = None
                continue
        except Exception:
            pass

        vals = [sanitize_text(v).lower() for v in row]
        if "folio" in vals and any("fecha venc" in v for v in vals):
            detail_idx = {idx: val for idx, val in enumerate(vals)}
            continue

        if not current_codigo or detail_idx is None:
            continue
        ...
        idx_folio = next((k for k, v in detail_idx.items() if v == "folio"), None)
        idx_fv = next((k for k, v in detail_idx.items() if "fecha venc" in v), None)
        idx_fecha = next((k for k, v in detail_idx.items() if v == "fecha" or "fecha factura" in v), None)
        idx_saldo = next((k for k, v in detail_idx.items() if "saldo" in v), None)
        idx_cond = next((k for k, v in detail_idx.items() if "condicion" in v or "condición" in v), None)
        idx_mon = next((k for k, v in detail_idx.items() if "moneda" in v), None)

        folio = sanitize_text(row[idx_folio]) if idx_folio is not None and idx_folio < len(row) else ""
        fv = _to_date_str(row[idx_fv]) if idx_fv is not None and idx_fv < len(row) else ""
        ff = _to_date_str(row[idx_fecha]) if idx_fecha is not None and idx_fecha < len(row) else ""
        saldo = _to_float(row[idx_saldo]) if idx_saldo is not None and idx_saldo < len(row) else 0.0
        cond = sanitize_text(row[idx_cond]) if idx_cond is not None and idx_cond < len(row) else ""
        mon = sanitize_text(row[idx_mon]) if idx_mon is not None and idx_mon < len(row) else ""
    ...
    if not out:
        return pd.DataFrame(columns=["Mes", "Codigo", "Folio", "Fecha_Factura", "Fecha_Vencimiento", "Saldo_Vence", "Condicion", "Moneda"])
    return pd.DataFrame(out)


def render_cobranza_tab():
    st.subheader("📒 Cobranza")
    spreadsheet = get_cobranza_spreadsheet()
    ws_base = spreadsheet.worksheet("cobranza_base")
    ws_venc = spreadsheet.worksheet("cobranza_vencimientos")
    ws_com = spreadsheet.worksheet("cobranza_comentarios")
    ...
    ensure_headers(ws_base, base_headers)
    ensure_headers(ws_venc, venc_headers)
    ensure_headers(ws_com, com_headers)

    st.markdown("### Cargar archivos del mes")
    c1, c2 = st.columns(2)
    now = datetime.now(TZ)
    with c1:
        anio = st.selectbox("Año", options=list(range(now.year - 2, now.year + 2)), index=2, key="cob_anio")
    with c2:
        mes_n = st.selectbox("Mes", options=list(range(1, 13)), index=now.month - 1, key="cob_mes")
    mes_sel = f"{anio}-{mes_n:02d}"

    reporte_file = st.file_uploader("REPORTE.xlsx", type=["xlsx"], key="cob_reporte")
    antig_file = st.file_uploader("ANTIGÜEDAD_SALDOS.xlsx", type=["xlsx"], key="cob_ant")
    if st.button("Procesar", key="btn_cob_procesar"):
        if reporte_file is None or antig_file is None:
            st.error("Carga ambos archivos para procesar.")
        else:
            df_base = parse_reporte_excel(reporte_file, mes_sel)
            df_venc = parse_antiguedad_excel(antig_file, mes_sel)
            clientes_base = set(df_base["Codigo"].astype(str))
            clientes_antig = set(df_venc["Codigo"].astype(str))
            no_encontrados = clientes_base - clientes_antig
            df_base["Tipo_Pago"] = np.where(df_base["Codigo"].astype(str).isin(no_encontrados), "CONTADO", "CREDITO")
            df_base["Ultima_Actualizacion"] = datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S")
            df_venc["Ultima_Actualizacion"] = datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S")

            upsert_rows_by_key(ws_base, df_base[base_headers], ["Mes", "Codigo"], ["Razon_Social", "Saldo", "No_Vencido", "Vencido", "Tipo_Pago", "Ultima_Actualizacion"])
            if not df_venc.empty:
                upsert_rows_by_key(ws_venc, df_venc[venc_headers], ["Codigo", "Folio", "Fecha_Vencimiento"], ["Mes", "Fecha_Factura", "Saldo_Vence", "Condicion", "Moneda", "Ultima_Actualizacion"])

            missing_df = df_base[df_base["Codigo"].astype(str).isin(no_encontrados)][["Codigo", "Razon_Social"]].copy()
            st.session_state["cobranza_missing_df"] = missing_df
            st.session_state["cobranza_stats"] = {
                "clientes": int(len(df_base)),
                "folios": int(len(df_venc)),
                "contado": int(len(missing_df)),
            }
            st.success("Proceso de cobranza completado.")

    stats = st.session_state.get("cobranza_stats")
    if stats:
        m1, m2, m3 = st.columns(3)
        m1.metric("Clientes base", stats["clientes"])
        m2.metric("Folios vencimientos", stats["folios"])
        m3.metric("Clientes CONTADO", stats["contado"])

    missing_df = st.session_state.get("cobranza_missing_df", pd.DataFrame(columns=["Codigo", "Razon_Social"]))
    if not missing_df.empty:
        st.warning("Clientes en REPORTE no encontrados en ANTIGÜEDAD (marcados como CONTADO).")
        st.dataframe(missing_df, use_container_width=True, hide_index=True)

    st.markdown("### Comentarios")
    mes_com = st.text_input("Mes comentarios (YYYY-MM)", value=mes_sel, key="cob_mes_com")
    base_records = pd.DataFrame(load_sheet_records_with_row_numbers(ws_base))
    clientes_mes = base_records[base_records.get("Mes", "").astype(str) == mes_com] if not base_records.empty else pd.DataFrame(columns=["Codigo", "Razon_Social"])
    if clientes_mes.empty:
        ...
        cli_sel = st.selectbox("Cliente", options=opciones, key="cob_cliente_sel")
        dia_sel = st.selectbox("Día", options=list(range(1, 32)), key="cob_dia_sel")
        comentario = st.text_area("Comentario", key="cob_comment_text")
        user_ini = st.text_input("Actualizado_por", key="cob_user_ini")
        if st.button("Guardar comentario", key="btn_cob_guardar"):
            codigo = cli_sel.split(" - ")[0].strip()
            df_comment = pd.DataFrame([
                {
                    ...
                    "Actualizado_por": user_ini,
                    "Timestamp": datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S"),
                }
            ])
            upsert_rows_by_key(ws_com, df_comment[com_headers], ["Mes", "Codigo", "Dia"], ["Comentario", "Actualizado_por", "Timestamp"])
            st.success("Comentario guardado correctamente.")

    st.markdown("### Descargar")
    mes_dl = st.text_input("Mes descarga (YYYY-MM)", value=mes_sel, key="cob_mes_dl")
    if st.button("Generar y descargar Excel", key="btn_cob_excel"):
        base_df = pd.DataFrame(load_sheet_records_with_row_numbers(ws_base))
        com_df = pd.DataFrame(load_sheet_records_with_row_numbers(ws_com))
        base_df = base_df[base_df.get("Mes", "").astype(str) == mes_dl].copy() if not base_df.empty else pd.DataFrame()
        if base_df.empty:
            st.error("No hay registros en cobranza_base para el mes seleccionado.")
        else:
            out = base_df[["Codigo", "Razon_Social"]].drop_duplicates().copy()
            for d in range(1, 32):
                out[str(d)] = ""
            if not com_df.empty:
                com_df = com_df[com_df.get("Mes", "").astype(str) == mes_dl].copy()
                for r in com_df.itertuples(index=False):
                    dia = sanitize_text(getattr(r, "Dia", "1")) or "1"
                    if dia in out.columns:
                        out.loc[out["Codigo"].astype(str) == sanitize_text(getattr(r, "Codigo", "")), dia] = sanitize_text(getattr(r, "Comentario", ""))

            mes_num = int(mes_dl.split("-")[1]) if "-" in mes_dl else now.month
            anio_num = int(mes_dl.split("-")[0]) if "-" in mes_dl else now.year
            buf = BytesIO()
            with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
                sheet_name = "Cobranza"
                out.to_excel(writer, index=False, sheet_name=sheet_name, startrow=1)
                ws = writer.sheets[sheet_name]
                ws.write(0, 0, f"Fecha De Generación: {datetime.now(TZ).strftime('%Y-%m-%d %H:%M:%S')}   Año: {anio_num}   Mes: {calendar.month_name[mes_num]}")
            buf.seek(0)
            st.download_button(
