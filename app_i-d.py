...
if selected_tab == 0:
    ...
    row1_col3.metric("🎟️ Ticket prom (global)", f"${ticket_prom:,.0f}")
    row1_col4.metric("👥 Clientes con historial", f"{evaluados:,}")

    row2_col1, row2_col2, row2_col3, row2_col4 = st.columns(4)
    row2_col1.metric("🆕 Nuevos/Sin historial", f"{nuevos:,}")
    row2_col2.metric("👥 Clientes totales actuales", f"{total_clientes_actuales:,}")
    row2_col3.metric("% cartera activa", f"{pct_activo * 100:.1f}%")
    row2_col4.metric("% cartera en riesgo", f"{pct_riesgo * 100:.1f}%")
