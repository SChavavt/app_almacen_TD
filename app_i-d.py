...
if selected_tab_key == "reportes_surtidores":
    ...
    st.caption("Métricas de productividad por día, semana y mes usando `data_pedidos` + `datos_pedidos`.")
    ...
    if not frames:
        ...
        if df_rep.empty:
            ...
            df_rep.loc[missing_dt, "_dt_registro"] = pd.to_datetime(df_rep.loc[missing_dt, "Fecha_Entrega"], errors="coerce")
            ...
            if total == 0:
                ...
                cols_show = [c for c in ["_origen", "_fecha", "Surtidor", "Folio_Factura", "Cliente", "Tipo_Envio", "Estado", "Turno", "Hora_Registro"] if c in df_f.columns]
