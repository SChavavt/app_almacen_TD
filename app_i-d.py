...
def render_auto_list(
    ...
    row_height_px = 24 if is_pantalla_l_local_view else (52 if is_large_auto_list_view else 38)
    title_height_px = (
        34
        if show_header and is_compact_pantalla_f_foraneo_view
        else (38 if show_header and is_pantalla_l_local_view else (58 if show_header else 10))
    )
    default_min_content = 86 if is_pantalla_l_local_view else (140 if show_header else 100)
    min_content = min_content_height if min_content_height is not None else default_min_content
    safety_padding_px = 10 if is_pantalla_l_local_view else 24
    content_height = max(
