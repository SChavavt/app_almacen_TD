...
if selected_tab_key == "auditores":
    ...
    foraneo_entries = sorted(foraneo_entries, key=lambda e: e.get("display_num", float("inf")))
    local_letter_map = {}
    ...
    completados_hoy = foraneo_entries + local_entries
    ...
    if not completados_hoy:
        ...
        for e in completados_hoy:
            ...
            if tipo_envio == "🚚 Pedido Foráneo":
                numero_foraneo = e.get("display_num", e.get("numero", ""))
                numero_foraneo_txt = sanitize_text(str(numero_foraneo))
            ...
            labels[key] = (
                ...
                f"{sanitize_text(e.get('estado', ''))} · 🧑‍🔧 {surtidor_actual}"
            )
        with st.form("auditores_asignacion_form"):
            ...
            selected_keys = []
            mitad = int(np.ceil(len(completados_hoy) / 2))
            left_entries = completados_hoy[:mitad]
            right_entries = completados_hoy[mitad:]
            col_left, col_right = st.columns(2, gap="large")
            with col_left:
